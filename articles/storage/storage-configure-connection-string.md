...
Jeden scénář, kde můžete chtít zadat explicitní koncový bod je, když jste změnili koncový bod úložiště objektů Blob [vlastní domény](storage-custom-domain-name.md). V takovém případě můžete zadat svůj vlastní koncový bod pro úložiště objektů Blob v připojovacím řetězci. Volitelně můžete zadat výchozí koncové body pro jiné služby, pokud je vaše aplikace používá.
