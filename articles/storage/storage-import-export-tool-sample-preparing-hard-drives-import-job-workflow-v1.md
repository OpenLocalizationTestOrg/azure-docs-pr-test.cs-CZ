...
Úloha importu Hello budou naimportovány tato data do hello následující cíle v účtu úložiště hello:  
...
V tomto příkladu by mělo být dostatečné dva 3TB pevné disky. Ale protože hello zdrojový adresář `H:\Video` má 5TB dat a jeden pevný disk je kapacita je jenom 3TB, je nutné toobreak `H:\Video` do dvou menší adresářů dřív, než spustíte hello nástroj Microsoft Azure Import/Export: `H:\Video1` a `H:\Video2`. Tento krok vypočítá hello následující zdrojového adresáře:  
...
|\\\bigshare\john\music|10GB|https://mystorageaccount.BLOB.Core.Windows.NET/Music|  
 ...
 Všimněte si, že i když hello `H:\Video`directory rozdělení tootwo adresáře, budou bodu toohello stejný cílový virtuální adresář v účtu úložiště hello. Tímto způsobem, všechny soubory videa se udržují v rámci jedné `video` kontejneru v účtu úložiště hello.  
  
 V dalším kroku hello výše zdroj, který adresáře jsou rovnoměrně distribuovat toohello dva pevné disky:  
...
## <a name="preparing-disk-for-import-when-data-is-pre-loaded"></a>Příprava disku pro import, když je předem načtená data
 ...
 Pokud toobe hello data importovat je již na disku hello, použijte příznak /skipwrite hello. Hodnota /t a /srcdir by měla odkazovat toohello disku připraveném pro import. Pokud všechny hello data na disku hello musí toogo toohello stejné cílový virtuální adresář nebo kořenové hello účtu úložiště, spusťte hello stejný příkaz pro každý adresář samostatně zachování hello hodnota /id stejné napříč všechny spustí.
...
Jakmile jste dokončili hello kopírování relací, můžete odpojit hello dvě jednotky z počítače kopie hello a dodávat je toohello příslušné služby Windows Azure datového centra. Budete nahrát hello dva soubory deníku, `FirstDrive.jrn` a `SecondDrive.jrn`, když vytvoříte hello úlohy importu v hello [Windows Azure Management Portal](https://manage.windowsazure.com/).  
