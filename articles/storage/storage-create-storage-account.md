...
Můžete také nakonfigurovat toouse název vlastní doménu se svým účtem úložiště. Pro účty úložiště Classic najdete informace v tématu věnovaném [konfiguraci vlastního názvu doménu pro koncový bod služby Blob Storage](storage-custom-domain-name.md). Pro účty správce prostředků úložiště, tato funkce nebyla přidána toohello [portál Azure](https://portal.azure.com) ještě, ale můžete ji nakonfigurovat v prostředí PowerShell. Další informace najdete v tématu hello [Set AzureRmStorageAccount](https://msdn.microsoft.com/library/mt607146.aspx) rutiny.  
...
5. Vyberte typ účtu úložiště hello: **obecné účely** nebo **úložiště objektů Blob**. **Obecné účely** je výchozí hello.
    ...
    Pokud **úložiště objektů Blob** byla vybrána, a zadejte úroveň přístupu hello: **horká** nebo **nástrojů**. Výchozí hodnota Hello je **horká**. Další podrobnosti najdete v tématu [Azure Blob Storage: úrovně Cool a Hot](storage-blob-storage-tiers.md).
...
Pro objekt Blob, které účty úložiště, změna úrovně přístupu hello mohou být účtovány poplatky za hello změní kromě toochanging cena za předplatné. Najdete v tématu hello [účty Blob storage – ceny a fakturace](storage-blob-storage-tiers.md#pricing-and-billing) další podrobnosti.
...
toodelete účet úložiště, který je přidružený virtuální počítač Azure, musíte napřed zajistit, aby byly odstraněny všechny disky virtuálního počítače. Pokud nejdříve neodstraníte disky virtuálního počítače, pak když zkusíte toodelete váš účet úložiště, zobrazí se chybová zpráva podobná:
...
Pokud účet úložiště hello používá model nasazení Classic hello, můžete odebrat hello disku virtuálního počítače pomocí následujících kroků v hello [portál Azure](https://manage.windowsazure.com):

1. Přejděte toohello [portál Azure classic](https://manage.windowsazure.com).
2. Přejděte karta toohello virtuálních počítačů.
3. Klikněte na kartu disky hello.
4. Vyberte datový disk a potom klikněte na Odstranit disk.
5. toodelete diskové Image, přejděte na kartu bitové kopie toohello a odstraňte všechny Image, které jsou uložené v účtu hello.

Další informace najdete v tématu hello [virtuální počítač Azure dokumentaci](http://azure.microsoft.com/documentation/services/virtual-machines/).
