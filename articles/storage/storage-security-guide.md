...
Zabezpečení dat roviny odkazuje na metodu sloužící k zabezpečení datové objekty uložené ve službě Azure Storage – objekty BLOB, fronty, tabulky a soubory. Jsme viděli metody k šifrování dat a zabezpečení během přenosu dat, ale jak tedy můžete o povolení přístupu k objektům?

V podstatě existují dvě metody pro řízení přístupu k objektům data sami. První je řízení přístupu ke klíči účtu úložiště a druhá používá sdílené přístupové podpisy k udělení přístupu k konkrétní datové objekty pro konkrétní množství času.

Všimněte si, jedinou výjimkou je, že můžete povolit veřejný přístup do objektů BLOB nastavením úroveň přístupu pro kontejner, který obsahuje objekty BLOB odpovídajícím způsobem. Pokud jste nastavili přístup pro kontejner objektů Blob nebo kontejner, vám umožní veřejný přístup pro čtení pro objekty BLOB v tomto kontejneru. To znamená, že každý, kdo má URL ukazující na objekt blob v tomto kontejneru ho otevřete v prohlížeči bez pomocí sdíleného přístupového podpisu nebo s klíče účtu úložiště.
...
* Jedná se o podrobné pokyny pro vytvoření sdílené přístupové podpisy a uložené zásady přístupu pomocí klientské knihovny .NET.
  ...
  * [Sdílené přístupové podpisy, část 2: Vytvoření a použití SAS s služby objektů Blob](storage-dotnet-shared-access-signature-part-2.md)
...
* Omezení přístupu pomocí IP adresy (IP ACL)
  ...
  * [Vytváření SAS služby](https://msdn.microsoft.com/library/azure/dn140255.aspx)
    ...
    Toto je odkaz na článek pro úroveň služby SAS; obsahuje příklad funkce ACLing IP.
  * [Vytváření SAS účtu](https://msdn.microsoft.com/library/azure/mt584140.aspx)

    Toto je odkaz na článek úrovni účtu SAS; obsahuje příklad funkce ACLing IP.
* Authentication
...
* [Začínáme s úložištěm Azure File ve Windows](storage-dotnet-how-to-use-files.md)
  ...
  Tento článek nabízí přehled Azure sdílené složky a jak připojit a používat je pomocí prostředí PowerShell a rozhraní .NET.
* [Uvnitř Azure File Storage](https://azure.microsoft.com/blog/inside-azure-file-storage/)

  Tento článek oznamuje obecné dostupnost Azure File storage a obsahuje technické informace o šifrování protokolu SMB 3.0.
