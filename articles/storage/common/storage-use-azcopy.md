...
Všimněte si, že pokud hello složky `C:\myfolder` neexistuje, vytvoří AzCopy a stažení `abc.txt ` do nové složky hello.
...
Po stažení operaci hello hello directory `C:\myfolder` zahrnuje hello následující soubory:
...
Po stažení operaci hello hello složky `C:\myfolder` zahrnuje hello následující soubory:

    C:\myfolder\abc.txt
    C:\myfolder\abc1.txt
    C:\myfolder\abc2.txt

Předpona Hello platí toohello virtuální adresář, který tvoří první část hello hello název objektu blob. Ve výše uvedeném příkladu hello virtuální adresář hello neodpovídá zadané předpony hello, takže nebude stažen. Kromě toho pokud hello možnost `\S` není zadán, AzCopy nestáhne žádné objekty BLOB.
...
Pokud hello zadaný virtuální adresář neexistuje, AzCopy odešle hello tooinclude hello virtuální adresář souboru ve svém názvu (*například*, `vd/abc.txt` v předchozím příkladu hello).
...
Zadání `/S` nahrávání hello obsah hello zadaný adresář tooBlob úložiště rekurzivně, což znamená, že jsou také odeslány všechny podsložky a jejich soubory. Například předpokládejme hello následující soubory jsou umístěny ve složce `C:\myfolder`:
...
Pokud nezadáte možnost `/S`, AzCopy nebyl odeslán rekurzivně. Po operaci odeslání hello hello kontejner obsahuje hello následující soubory:
...
Pokud zadáte `/SetContentType` bez hodnoty AzCopy nastaví jednotlivých objektů blob nebo typ obsahu souboru podle tooits příponu souboru.
...
Po operaci kopírování hello hello cílový kontejner obsahuje hello blob a jeho snímků. Za předpokladu, že objektu blob hello v předchozím příkladu hello má dva snímky, zahrnuje následující hello hello kontejner objektů blob a snímky:
...
AzCopy ve výchozím nastavení zkopíruje data mezi dva koncové body úložiště asynchronně. Proto se zkopíruje spustí operace kopírování hello hello pozadí využití kapacity přebytečné šířky pásma, která nemá žádné SLA z hlediska jak rychlé objekt blob a AzCopy pravidelně kontroluje stav kopie hello kopírování hello je splněna nebo se nezdařilo.
...
Pokud hello zadaný zdroj je sdílenou složku Azure a pak buď zadejte hello přesný název souboru, (*například* `abc.txt`) toodownload jeden soubor, nebo zadejte možnost `/S` toodownload všechny soubory ve sdílené složce hello rekurzivní. Probíhá pokus toospecify vzor souborů i možnost `/S` společně dojde k chybě.
...
Můžete zadat hello `/SyncCopy` možnost toocopy data z úložiště File tooFile úložiště, ze souboru úložiště tooBlob úložiště a z úložiště objektů Blob tooFile úložiště synchronně, AzCopy k tomu stahování hello zdroje dat toolocal paměti a nahrajte ho znovu toodestination. Standardní výstupní náklady platí.
...
index klíče rozsahu oddílu Hello je 0, pokud uživatel hello není nastavena možnost `/PKRS`.
...
Všimněte si, že hello minimální možné hodnoty pro možnost `/SplitSize` je 32 MB. Pokud hello zadaný cíl je úložiště objektů Blob, AzCopy rozdělí hello datový soubor po jeho velikosti dosáhne omezení velikosti objektu blob hello (200GB), bez ohledu na tom, jestli možnost `/SplitSize` byla zadána uživatelem hello.
...
Při zadávání Formát datové části hello sdíleného svazku clusteru, AzCopy taky generuje soubor schématu s příponou `.schema.csv` pro každý datový soubor.
...
AzCopy spustí souběžných operací tooexport entity, když uživatel hello určuje možnost `/PKRS`. Každé operace exportuje jeden rozsah klíče oddílu.
...
AzCopy generuje soubor dat JSON do kontejneru objektů blob hello následující zásady vytváření názvů:
...
AzCopy selže, pokud rozdělení hello parametr mezi dvěma čárami, jak je vidět tady pro hello `/sourcekey` parametr:
...
Pokud hello deníku soubor neexistuje, AzCopy kontroluje, zda text hello příkazový řádek, který můžete vložit odpovídá hello příkazového řádku v souboru deníku hello. Pokud se dva příkazy na příkazových řádcích hello shodují, obnoví AzCopy hello nedokončené operace. Pokud se neshodují, jste výzvami tooeither přepsat hello deníku souboru toostart operaci nového nebo toocancel hello aktuální operace.
...
Pokud hello zadaný zdroj je sdílenou složku Azure a pak buď zadejte hello přesný název souboru, (například abc.txt) toocopy jeden soubor, nebo zadejte možnost /S toocopy všechny soubory v rekurzivně hello sdílené složky. Pokus toospecify obou soubor vzor a možnost /S společně dojde k chybě.
...
Pokud hello zdroje prostředků je kontejner objektů blob a je k dispozici klíč ani SAS, je pro kontejner objektů blob hello čtení přes anonymní přístup.
...
Ve výchozím nastavení, je hello podrobný soubor protokolu s názvem AzCopyVerbose.log v `%LocalAppData%\Microsoft\Azure\AzCopy`. Pokud zadáte existující umístění souborů pro tuto možnost, podrobného protokolování hello je připojením toothat souboru.  
...
Pokud není tato možnost zadána, nebo je zadán bez cestu ke složce, AzCopy se vytvoří soubor deníku hello hello výchozího umístění, což je % LocalAppData%\Microsoft\Azure\AzCopy.
...
AzCopy interpretuje hello použití této možnosti simulace pro spuštěné hello příkazového řádku bez možnost /L a počty se zkopírují jak mnoho objektů, můžete zadat možnost /V na stejný čas toocheck objektů, které jsou zkopírovány v podrobného protokolování hello hello.
...
### <a name="xn"></a>/XN
Vyloučí novější zdrojovému prostředku. Hello prostředků není zkopírována, pokud hello čas poslední změny zdroje hello hello stejnou nebo novější, než cílový.

**Platí pro:** objekty BLOB, soubory

### <a name="xo"></a>/XO
Vyloučí starší zdrojovému prostředku. Hello prostředků není zkopírována, pokud hello čas poslední změny zdroje hello hello stejné nebo starší než cílový.
...
Pokud není tato možnost zadána, exportuje AzCopy jedním souborem tooa dat tabulky.
...
Tato možnost je volitelné během operace exportu hello, AzCopy vygeneruje soubor manifestu pomocí předdefinovaných název, pokud není tato možnost zadána.
...
Pokud zadáte tuto možnost bez hodnoty, AzCopy nastaví jednotlivých objektů blob nebo typ obsahu souboru podle tooits příponu souboru.
...
Pro vlastnost "AzureStorageUseV1MD5" • True – hello výchozí hodnota AzCopy používá implementace rozhraní .NET MD5.
...
* [Jak toouse úložiště Blob z rozhraní .NET](../blobs/storage-dotnet-how-to-use-blobs.md)
* [Jak toouse úložiště File z rozhraní .NET](../storage-dotnet-how-to-use-files.md)
* [Jak toouse úložiště Table z rozhraní .NET](../../cosmos-db/table-storage-how-to-use-dotnet.md)
