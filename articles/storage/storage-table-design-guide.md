...
V této části jsou zdůrazněné některé hello klíčové funkce služby Table hello které jsou obzvláště důležité toodesigning pro výkon a škálovatelnost. Pokud jste tooAzure nové úložiště a hello služby Table, nejdřív přečíst [tooMicrosoft Úvod Azure Storage](storage-introduction.md) a [Začínáme s Azure Table Storage pomocí rozhraní .NET](storage-dotnet-how-to-use-tables.md) než si přečtete zbytek hello článek. I když je aktivní hello tohoto průvodce hello služby Table, bude obsahovat některé diskuzi o hello fronty Azure a objekt Blob služby a jak je možné použít společně s hello služby Table v řešení.  
...
Informace o šifrování dat v tabulce najdete v tématu [šifrování na straně klienta a Azure Key Vault pro Microsoft Azure Storage](storage-client-side-encryption.md).  
...
Další informace o používání tokeny SAS s hello služby Table najdete v tématu [pomocí sdíleného přístupového podpisy (SAS)](storage-dotnet-shared-access-signature-part-1.md).  
