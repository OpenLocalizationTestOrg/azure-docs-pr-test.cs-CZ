...
V tomto kurzu rozšíříte vytvořené v aplikaci [webové aplikace Node.js pomocí Express] kurz pomocí knihovny klienta Microsoft Azure pro platformu Node.js pro práci se službami pro správu dat. Rozšíříte aplikace k vytvoření aplikace založené na webu – seznam úloh, kterou můžete nasadit do Azure. Seznam úloh umožňuje uživateli načíst úlohy, přidat nové úkoly a úkoly označit jako dokončená.

Položky úkolů jsou uložené ve službě Azure Storage. Úložiště Azure poskytuje úložiště nestrukturovaných dat, které je odolné proti chybám a vysoce dostupné. Úložiště Azure obsahuje několik datové struktury, kam můžete ukládat a přistupovat k datům a mohou využívat služby storage z rozhraní API zahrnutý v sadě Azure SDK pro Node.js nebo přes rozhraní REST API. Další informace najdete v tématu [ukládání a přístup k datům v Azure].
...
Pro přístup k úložišti Azure, musíte předat přihlašovací údaje úložiště. K tomu využívat nastavení aplikace v souboru web.config.
Tato nastavení budou předány jako proměnné prostředí do uzlu, které jsou pak přečte sadu Azure SDK.

> [!NOTE]
> Přihlašovací údaje úložiště používají jenom při nasazení aplikace do Azure. Při spuštění v emulátoru, aplikace bude používat emulátor úložiště.
>
>
...
3. V okně Azure Powershell zadejte následující rutiny můžete načíst informace o účtu úložiště:
   ...
   To umožňuje načíst seznam účtů úložiště a účtu klíče přidruženého k vaší hostované služby.
...
V této části rozšíříte základní aplikace vytvořené **express** příkaz přidáním **task.js** soubor, který obsahuje model pro úkoly. Také upravíte existující **app.js** a vytvořte novou **tasklist.js** soubor, který používá model.

### <a name="create-the-model"></a>Vytvoření modelu
1. V **WebRole1** adresáře, vytvořte nový adresář s názvem **modely**.
2. V **modely** adresáře, vytvořte nový soubor s názvem **task.js**. Tento soubor bude obsahovat model pro úkoly vytvořené v aplikaci.
...
4. Dále přidáte kód, který definuje export objektu Task. Tento objekt je zodpovědná za připojení do tabulky.
...
2. Do souboru **tasklist.js** přidejte následující kód: Tento kód načte moduly azure a async, které jsou používány **tasklist.js**. Také definuje **TaskList** funkce, která se předá instance **úloh** definovaného dříve:
...
3. V souboru app.js posuňte se dolů, kde uvidíte následující řádek:
    ...
    Nahraďte kód vidíte níže uvedených řádků. To bude inicializovat instanci <strong>úloh</strong> s připojením k účtu úložiště. To je předán <strong>TaskList</strong>, který bude používat pro komunikaci se službou tabulky:
...
2. Nahraďte obsah **index.jade** souboru pomocí kódu níže. Definuje zobrazení pro zobrazení stávající úlohy a také formuláře pro přidání nové úlohy a označit stávajících jako dokončenou.
...
Soubor **layout.jade** v adresáři **views** slouží jako globální šablona pro ostatní soubory **.jade**. V tomto kroku jej upravíte tak, aby používal [Twitter Bootstrap](https://github.com/twbs/bootstrap). To je sada nástrojů, které usnadňují návrh pěkných webů.
...
2. Z **zobrazení** složku, otevřete **layout.jade** v textovém editoru a nahraďte jeho obsah následujícím kódem:
...
V prohlížeči se otevře a zobrazí následující stránka:
...
Jako předtím protože jste zadali **– spusťte** možnost, otevře se prohlížeč a zobrazí vaši aplikaci běžící v Azure po dokončení publikování.

![Okno prohlížeče zobrazující stránku Moje seznamu úloh. Adresa URL značí, že stránky je teď hostované v Azure.](./media/storage-nodejs-use-table-storage-cloud-service-app/getting-started-1.png)
