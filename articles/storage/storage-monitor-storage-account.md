...
1. Vyberte hello **typ** metriky dat pro každou **služby** chcete toomonitor a hello **zásady uchovávání informací** pro hello data. Můžete také zakázat monitorování nastavením **stav** příliš**vypnout**.
   ...
   * **Na rozhraní API**: V přidání toohello agregovaná metrika, shromažďuje hello stejnou sadu metriky pro každé operace úložiště v hello rozhraní API služby Azure Storage.
   ...
   > [!WARNING]
   > Budou se vám účtovat, když ručně odstranit data metriky. Zastaralé analytická data (data starší než vaše zásady uchovávání informací) je odstraněn hello systému bez nákladů. Doporučujeme vám, nastavení zásady uchovávání informací založené na tom, jak dlouho chcete tooretain úložiště analytická data pro váš účet. V tématu [co poplatky, proveďte nimž dochází při povolení úložiště metriky?](storage-enable-and-view-metrics.md#what-charges-do-you-incur-when-you-enable-storage-metrics) Další informace.
   >
...
> [!NOTE]
> Azure Storage používá [tabulky úložiště](storage-introduction.md#table-storage) toostore hello metriky pro váš účet úložiště a metriky hello úložiště v tabulkách ve vašem účtu. Další informace najdete v tématu. [Ukládání metriky](storage-analytics.md#how-metrics-are-stored).
>
