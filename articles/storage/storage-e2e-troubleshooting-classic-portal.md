...
V tomto kurzu jsme ukazují, jak identifikovat klienta určitým chybám, které mohou ovlivnit výkon a řešení těchto chyb od začátku do konce pomocí nástroje poskytované společností Microsoft a Azure Storage, chcete-li optimalizovat klientské aplikace.
...
* **Portál Azure Classic**. Můžete nakonfigurovat protokolování a metriky pro svůj účet úložiště [portálu Azure Classic](https://manage.windowsazure.com). Můžete také zobrazit grafy, které ukazují, jak se vaše aplikace provádí v čase a konfigurovat výstrahy pro upozornění, pokud aplikace provede odlišně, než se očekávalo příslušné metriky.

    V tématu [monitorování účtu úložiště na portálu Azure](storage-monitor-storage-account.md) informace o konfiguraci monitorování na portálu Azure Classic.
...
Všimněte si, že následující seznamy daleko od dokončení. V tématu [stavu a kódy chyb](http://msdn.microsoft.com/library/azure/dd179382.aspx) podrobnosti o obecných chyb Azure Storage a informace o chybách, které jsou specifické pro jednotlivé služby úložiště.
...
První budeme potřebovat ke konfiguraci protokolování Azure Storage a metriky, tak, aby jsme data z aplikace klienta k analýze. Protokolování a metriky v mnoha různými způsoby – můžete nakonfigurovat přes [portálu Azure Classic](https://manage.windowsazure.com), pomocí prostředí PowerShell, nebo prostřednictvím kódu programu. V tématu [povolení metriky úložiště a zobrazení dat metrik](http://msdn.microsoft.com/library/azure/dn782843.aspx) a [povolení protokolování úložiště a přístup k datům protokolu](http://msdn.microsoft.com/library/azure/dn782840.aspx) podrobnosti o konfiguraci protokolování a metriky.

**Prostřednictvím portálu Azure Classic**

Chcete-li konfigurovat protokolování a metriky pro váš účet úložiště pomocí portálu, postupujte podle pokynů v [monitorování účtu úložiště na portálu Azure](storage-monitor-storage-account.md).

> [!NOTE]
> Není možné nastavit minutu metriky pomocí portálu Azure Classic. Doporučujeme však, že je nastavena pro účely tohoto kurzu a zkoumání problémů s výkonem s vaší aplikací. Můžete nastavit minutu metriky pomocí prostředí PowerShell, jak je znázorněno níže, nebo prostřednictvím kódu programu, nebo prostřednictvím portálu Azure Classic.
>
> Všimněte si, že na portálu Azure Classic nemůže zobrazit minutu metriky, pouze hodinové metriky.
>
>
...
## <a name="review-metrics-data-in-the-azure-classic-portal"></a>Zkontrolujte data metriky v portálu Azure Classic
Po spuštění vaší aplikace pro určitou dobu, můžete zkontrolovat grafy metriky, které se zobrazují v portálu Azure Classic sledovat, jak má byla provádění služby. Nejprve přidáme **procento úspěšnosti** metriky na stránku monitorování:

1. Přejděte na řídicí panel pro svůj účet úložiště [portálu Azure Classic](https://manage.windowsazure.com), pak vyberte **monitorování** zobrazíte monitorování stránku.
2. Klikněte na tlačítko **přidat metriky** zobrazíte **zvolte metriky** dialogové okno.
3. Posuňte se dolů a najděte **procento úspěšnosti** skupiny, rozbalte ho a pak vyberte **agregační**, jak je znázorněno na obrázku níže. Tato metrika agreguje data procento úspěšnosti z operace všech objektů Blob.

![Zvolte metriky](./media/storage-e2e-troubleshooting-classic-portal/choose-metrics-portal-1.png)

Na portálu Azure Classic zobrazí **procento úspěšnosti** v monitorování grafu, společně s další metriky jste mohli přidat (až šest lze zobrazit v grafu najednou). Na obrázku vidíte, že procento úspěšnosti je trochu nižší než 100 %, což je scénáře, který jsme vám další šetření tak, že analýza protokolů v Message Analyzer:

![Metriky grafu portálu](./media/storage-e2e-troubleshooting-classic-portal/portal-metrics-chart-1.png)

Další informace o přidání metriky na stránku sledování najdete v tématu [postupy: Přidání metriky do tabulky se metriky](storage-monitor-storage-account.md#add-metrics-charts-to-the-portal-dashboard).

> [!NOTE]
> To může trvat nějakou dobu metriky dat, aby se po povolení metrik úložiště zobrazí v portálu Azure Classic. To je proto hodinové metriky pro do předchozí hodiny nejsou zobrazeny v portálu Azure Classic, dokud neuplyne do aktuální hodiny. Navíc minutu metriky nejsou zobrazeny v portálu Azure Classic. Proto podle toho, když povolíte metriky, může trvat až dvě hodiny se zobrazí data metriky.
>
>
