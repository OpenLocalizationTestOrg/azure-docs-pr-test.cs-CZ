...
Po vytvoření prostředky budete přesměrováni na okně skupiny prostředků.
...
Kód v tomto příkladu se připojí k Kafka zprostředkovatele hostitelů Kafka clusteru. Když Pokud chcete najít Kafka zprostředkovatele hostitele, použijte následující příklad PowerShell nebo Bash:
...
> [!NOTE]
> Tento příklad předpokládá, že `$PASSWORD` obsahovat heslo pro přihlášení clusteru, a `$CLUSTERNAME` tak, aby obsahovala název clusteru Kafka.
>
