...
Při vytváření clusteru služby HDInsight, 6 tabulky se vytvářejí automaticky pro clustery se systémem Linux ve službě Table storage výchozí:
...
* hadoopinstalllog: protokolu událostí výjimek/došlo při instalaci Hadoop v clusteru. Tato tabulka může být užitečné při ladění problémy související s clustery jsou vytvořené pomocí vlastních parametrů.
* hadoopservicelog: protokolu události nebo výjimek zaznamenaného všechny služby Hadoop. Tato tabulka může být užitečné při ladění problémy související s selhání úloh v clusterech prostředí HDInsight.

Názvy souborů tabulky jsou **u<ClusterName>DDMonYYYYatHHMMSSsss<TableName>**.

Tyto tabulky obsahuje následující pole:
...
Power Query lze nainstalovat z [www.microsoft.com/en-us/download/details.aspx?id=39379](http://www.microsoft.com/en-us/download/details.aspx?id=39379). Naleznete na stránce stahování pro požadavky na systém

**Pomocí doplňku Power Query otevřete a analyzovat protokol služby**
...
5. Klikněte pravým tlačítkem na tabulky hadoopservicelog **Navigátor** panelu a vyberte **upravit**. Zobrazí se 4 sloupce. Volitelně můžete odstranit **klíč oddílu**, **klíč řádku**, a **časové razítko** sloupce je vyberete a poté na **odebrat sloupce** z možností na pásu karet.
...
Nyní můžete aplikaci Excel k filtrování a řazení podle potřeby. Chcete samozřejmě zahrnout ostatních sloupců (např. zprávy) Chcete-li rozbalit problémy, když k nim dojde, ale výběru a seskupování sloupců popsané výše poskytuje dostatečnou přehled o co se děje s služby Hadoop. Na stejné nápad může být použita setuplog a hadoopinstalllog tabulky.
...
Z portálu Azure klikněte na název clusteru HDInsight a otevřete okno clusteru. V okně clusteru, klikněte na tlačítko **řídicí panel**.

![Spusťte řídicí panel clusteru](./media/hdinsight-debug-jobs/hdi-debug-launch-dashboard.png)

Po zobrazení výzvy zadejte přihlašovací údaje Správce clusteru. V konzole dotazu, které se otevře, klikněte na **uživatelského rozhraní Hadoop**.
...
Z portálu Azure klikněte na název clusteru HDInsight a otevřete okno clusteru. V okně clusteru, klikněte na tlačítko **řídicí panel**. Po zobrazení výzvy zadejte přihlašovací údaje Správce clusteru. V konzole dotazu, které se otevře, klikněte na **uživatelském rozhraní YARN**.

Rozhraní YARN můžete provést následující akce:

* **Získat stav clusteru**. V levém podokně rozbalte **clusteru**a klikněte na tlačítko **o**. Tato přítomen clusteru podrobnosti o stavu jako přidělené paměti jádra používá, celkový stav správce prostředků clusteru, cluster verze atd.
...
Z portálu Azure klikněte na název clusteru HDInsight HBase a otevřete okno clusteru. V okně clusteru, klikněte na tlačítko **řídicí panel**. Po zobrazení výzvy zadejte přihlašovací údaje Správce clusteru. V konzole dotazu, které se otevře, klikněte na **uživatelského rozhraní HBase**.

## <a name="hdinsight-error-codes"></a>Kódy chyb HDInsight
Chybové zprávy uvedeno v této části jsou uvedené v Azure HDInsight Hadoop vysvětlení možné chybové stavy, které narazí při správě služby pomocí prostředí Azure PowerShell uživatele a poradit podle kroků, které můžete provést obnovení z chyby.
