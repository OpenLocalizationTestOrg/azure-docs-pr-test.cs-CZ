...
1. Použití `hive` příkaz ke spuštění prostředí hive. Měli byste vidět `hive>` výzvu po načetl prostředí.

2. Zadejte následující dotaz na `hive>` řádku:
...
1. Použití `pig` příkaz ke spuštění prostředí. Zobrazí `grunt>` výzvu po načetl prostředí.
...
5. Jednou v editoru, zrušte komentář u následujícího řádku odebráním `#` znak od začátku řádku:
    ...
    Jakmile má byly provedeny změny, ukončete editor pomocí kombinace kláves Ctrl + X. Vyberte Y a potom zadejte a uložte změny.
