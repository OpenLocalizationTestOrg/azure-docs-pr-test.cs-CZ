...
Tento příklad znázorňuje životního cyklu aplikace Service Fabric prostřednictvím volání rozhraní REST API. Další informace o životním cyklu aplikace Service Fabric najdete v tématu [životního cyklu aplikace Service Fabric](service-fabric-application-lifecycle.md).

Tato ukázka provede následující akce:

* Zřizuje **WordCount 1.0.0** ukázku z balíčku aplikace WordCount v úložišti bitové kopie.
* Zobrazí seznam typů aplikací, která zahrnuje WordCount 1.0.0.
* Vytvoří aplikaci WordCount jako **fabric: / WordCount**.
* Zobrazí seznam aplikací, která zahrnuje fabric: / WordCount verze 1.0.0.
* Zřizuje 1.1.0 verzi příkladu WordCount z **WordCountUpgrade** balíčku aplikace v úložišti bitové kopie.
* Zobrazí seznam typů aplikací, které zahrnuje i WordCount 1.0.0 a **WordCount 1.1.0**.
* Upgraduje na verzi 1.1.0 aplikaci WordCount.
* Zobrazí seznam aplikací, které zahrnuje WordCount verze 1.1.0, ale už obsahuje WordCount verze 1.0.0.
* Odstraní aplikaci WordCount.
* Zobrazí seznam aplikací, už zahrnující fabric: / WordCount.
* Verze unprovisions 1.1.0 vzorku WordCount.
* Zobrazí seznam typů aplikací, které zahrnuje WordCount 1.0.0, ale už obsahuje WordCount 1.1.0.
* Verze unprovisions 1.0.0 vzorku WordCount.
* Zobrazí seznam typy aplikací, které už obsahuje WordCount.

## <a name="prerequisites"></a>Požadavky
Této ukázce se používá [WordCount ukázkové](http://aka.ms/servicefabricsamples) (v nalezen **Začínáme** ukázky). Ukázka WordCount musí být nejprve sestaven, a pak dva balíčky aplikací, musí se zkopírovat do úložiště bitové kopie.

| Složka | Popis |
| --- | --- |
| WordCount |Ukázkovou aplikaci WordCount. **ApplicationManifest.xml** soubor obsahuje **ApplicationTypeVersion = "1.0.0"**. |
| WordCountUpgrade |Ukázkovou aplikaci WordCount. Soubor ApplicationManifest.xml musí změnit tak, aby **ApplicationTypeVersion = "1.1.0"** umožňující upgradu aplikace mohla probíhat. |

Při vytváření balíčků aplikací a zkopírujte je do úložiště bitové kopie, proveďte následující kroky:

1. Kopírování **C:\ServiceFabricSamples\Services\WordCount\WordCount\pkg\Debug** k **C:\Temp\WordCount**. Tím se vytvoří balíček aplikace WordCount.
2. Zkopírujte C:\Temp\WordCount k **C:\Temp\WordCountUpgrade**. Tím se vytvoří **WordCountUpgrade aplikace** balíčku.
3. Otevřete **C:\Temp\WordCountUpgrade\ApplicationManifest.xml** v textovém editoru.
4. V **ApplicationManifest** elementu, změny **ApplicationTypeVersion** atribut **"1.1.0"**.  Tím se aktualizuje číslo verze aplikace.
5. Uložte soubor změněné ApplicationManifest.xml.
6. Spusťte následující skript prostředí PowerShell jako správce a kopírovat aplikace do úložiště bitové kopie:
...
Po dokončení skriptu prostředí PowerShell je připraven ke spuštění této aplikace.

## <a name="example"></a>Příklad
Následující příklad ukazuje životního cyklu aplikace Service Fabric.
