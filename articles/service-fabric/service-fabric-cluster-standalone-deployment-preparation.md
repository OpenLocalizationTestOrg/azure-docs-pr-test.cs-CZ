...
Chystáte se vytvořit cluster Service Fabric na počítačích, které vlastníte, abyste se mohli rozhodnout, jaké druhy chyb má cluster situaci zvládnout. Například potřebujete samostatné řádky napájení nebo připojení k Internetu zadaný do těchto počítačů? Kromě toho vezměte v úvahu fyzické zabezpečení těchto počítačů. Kde se nacházejí na počítače, a který potřebuje přístup k nim? Po provedení těchto rozhodnutí, můžete namapovat na počítače logicky na různých domén selhání (viz krok 4). Plánování pro produkční clustery infrastruktury je složitější než pro testovacích clusterů.

### <a name="step-2-prepare-the-machines-to-meet-the-prerequisites"></a>Krok 2: Příprava počítačů ke splnění předpokladů
Předpoklady pro každý počítač, který chcete přidat do clusteru:
...
### <a name="step-3-determine-the-initial-cluster-size"></a>Krok 3: Určení počáteční velikost
Každý uzel v clusteru Service Fabric samostatné má Service Fabric runtime nasadit a je členem clusteru. V typické provozní nasazení je jeden uzel jednu instanci OS (fyzické nebo virtuální). Velikost clusteru je určen podle obchodních potřeb. Ale musí mít velikost minimální clusteru tři uzly (počítačů nebo virtuálních počítačů).
...
### <a name="step-5-download-the-service-fabric-standalone-package-for-windows-server"></a>Krok 5: Stáhněte si balíček Service Fabric samostatné pro systém Windows Server
...
Pokud správce clusteru konfiguruje samostatný cluster Service Fabric, je v prostředí musí být nastavení s těmito kritérii: <br>
...
* Případně pokud přístup k síti internet, je omezený na uvedených v seznamu povolených domén, domén níže jsou požadovány pro automatický upgrade: go.microsoft.com download.microsoft.com
...
> [!NOTE]
> Průběžně vydáváme zlepšení robustnější, aby tento modul, pokud se nestane poškozené nebo chybí, které budete mít dojem, není aktuálně zachytila TestConfiguration, dejte nám vědět prostřednictvím našich [podporují kanály](https://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-support).   
> 
> 
