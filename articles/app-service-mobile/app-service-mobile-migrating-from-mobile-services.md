...
* Připojení ke své místní prostředky pomocí [VNet] kromě [hybridní připojení].
...
Tyto přihlašovací údaje můžete použít k lokalitě pomocí git clone nebo nastavit automatické nasazení z Githubu, sady TFS nebo Mercurial.  Další informace najdete v tématu [dokumentaci k nasazení služby Azure App Service].
...
Vaše úlohy na vyžádání se nacházejí v `App_Data/config/scripts/scheduler post-migration`.  Doporučujeme vám, že převedete všechny úlohy na vyžádání do [WebJobs] nebo [funkce].  Zápis nových úloh plánovače jako [WebJobs] nebo [funkce].
