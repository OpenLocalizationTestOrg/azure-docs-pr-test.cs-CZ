---
title: "přesměrování aaaConfigure pro Azure Application Gateway - prostředí PowerShell | Microsoft Docs"
description: "Tato stránka obsahuje přesměrování tooconfigure scénáře pro službu Application Gateway pomocí prostředí PowerShell"
...
---
# <a name="configure-redirection-on-application-gateway-with-powershell"></a><span data-ttu-id="a6368-103">Konfigurace přesměrování ve službě Application Gateway pomocí PowerShellu</span><span class="sxs-lookup"><span data-stu-id="a6368-103">Configure redirection on Application Gateway with PowerShell</span></span>

<span data-ttu-id="a6368-104">Aplikační brána podporuje hello možnost tooredirect provozu na základě definovaných konfigurace.</span><span class="sxs-lookup"><span data-stu-id="a6368-104">Application gateway supports hello ability tooredirect traffic based on a defined configuration.</span></span> <span data-ttu-id="a6368-105">Další informace o přesměrování v obecné návštěvu toolearn [přehled přesměrování Application Gateway](application-gateway-redirect-overview.md).</span><span class="sxs-lookup"><span data-stu-id="a6368-105">toolearn more about redirection in general visit, [Application Gateway redirect overview](application-gateway-redirect-overview.md).</span></span> <span data-ttu-id="a6368-106">Tento článek obsahuje příklady tooHTTPS přesměrování protokolu HTTP, na základě cest přesměruje, přesměrování více lokalit a přesměruje tooexternal lokalit.</span><span class="sxs-lookup"><span data-stu-id="a6368-106">This article provides examples of HTTP tooHTTPS redirection, path based redirects, multi-site redirects, and redirects tooexternal sites.</span></span>

## <a name="http-toohttps-redirect-on-an-existing-application-gateway"></a><span data-ttu-id="a6368-107">Přesměrování protokolu HTTP tooHTTPS na existující aplikační brány</span><span class="sxs-lookup"><span data-stu-id="a6368-107">HTTP tooHTTPS redirect on an existing application gateway</span></span>

<span data-ttu-id="a6368-108">Hello následující příklad vytvoří nový naslouchací proces protokolu HTTP na portu 80 tooredirect požadavky toohello existující naslouchání na portu 443.</span><span class="sxs-lookup"><span data-stu-id="a6368-108">hello following example creates a new HTTP listener on port 80 tooredirect requests toohello existing listener on port 443.</span></span>
...
<span data-ttu-id="a6368-109">Jakmile se aktualizuje hello aplikační bránu, testovat hello nové naslouchání na portu 80.</span><span class="sxs-lookup"><span data-stu-id="a6368-109">Once hello application gateway is updated, test hello new listener on port 80.</span></span>  <span data-ttu-id="a6368-110">Hello následující příklad ukazuje použití `curl` testování hello přesměrování.</span><span class="sxs-lookup"><span data-stu-id="a6368-110">hello following example shows using `curl` test hello redirection.</span></span>
...
## <a name="path-based-redirect"></a><span data-ttu-id="a6368-111">Přesměrování na základě cest</span><span class="sxs-lookup"><span data-stu-id="a6368-111">Path based redirect</span></span>

<span data-ttu-id="a6368-112">Hello následující ukázka vytvoří nový a pravidlo cesty založené na aplikační brány, který přesměruje jenom provoz splňuje hello konkrétní cestu adresy url.</span><span class="sxs-lookup"><span data-stu-id="a6368-112">hello following example creates a new listener and a path based rule on an existing application gateway that redirects only traffic meeting hello specific url path.</span></span>
...
## <a name="multi-site-redirect"></a><span data-ttu-id="a6368-113">Přesměrování pro více webů</span><span class="sxs-lookup"><span data-stu-id="a6368-113">Multi-site redirect</span></span>

<span data-ttu-id="a6368-114">Hello následující příklad vytvoří novou aplikační bránu s 2 více lokalit moduly pro naslouchání na portu 80.</span><span class="sxs-lookup"><span data-stu-id="a6368-114">hello following example creates a new application gateway with 2 multi-site listeners on port 80.</span></span> <span data-ttu-id="a6368-115">moduly pro naslouchání Hello jsou pro adatum.com a adatum.org. Přesměrování se pravidlo vytvoří tooredirect provoz z adatum.org tooadatum.com. Další konfigurace je nutná pro konfiguraci CNAME aliasy toohello aplikace veřejná IP adresa brány, další informace o delegování tooAzure vaší domény DNS a vytvořením CNAME záznamy pro vaši doménu navštěvovat, [delegování domény tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md).</span><span class="sxs-lookup"><span data-stu-id="a6368-115">hello listeners are for adatum.com and adatum.org. A redirect rule is created tooredirect traffic from adatum.org tooadatum.com. Additional configuration is required for configuring CNAME aliases toohello application gateway public IP address, for more information on delegating your domain tooAzure DNS and creating CNAME records for your domain visit, [Delegate a domain tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md).</span></span>
...
## <a name="redirect-tooan-external-site"></a><span data-ttu-id="a6368-116">Přesměrování tooan externího webu</span><span class="sxs-lookup"><span data-stu-id="a6368-116">Redirect tooan external site</span></span>

<span data-ttu-id="a6368-117">Hello následující příklad přesměruje provoz přicházející do hello aplikace brány tooan externí web (vyhledávače bing.com).</span><span class="sxs-lookup"><span data-stu-id="a6368-117">hello following example redirects traffic coming into hello application gateway tooan external website (bing.com).</span></span> <span data-ttu-id="a6368-118">Při použití hello `TargetUrl` přepínač hello `IncludePath` a `IncludeQuery` přepínače nejsou povoleny.</span><span class="sxs-lookup"><span data-stu-id="a6368-118">When using hello `TargetUrl` switch hello `IncludePath` and `IncludeQuery` switches are not allowed.</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="a6368-119">Další kroky</span><span class="sxs-lookup"><span data-stu-id="a6368-119">Next steps</span></span>

<span data-ttu-id="a6368-120">Navštivte [end tooend SSL nakonfigurovat application gateway pomocí prostředí PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span><span class="sxs-lookup"><span data-stu-id="a6368-120">Visit [configure end tooend SSL with application gateway using PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span></span>
