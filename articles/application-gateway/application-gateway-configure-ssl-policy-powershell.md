...
Zjistěte, jak verze zásad tooconfigure SSL a ve Application Gateway sady šifer. Můžete vybrat [seznam předdefinovaných zásad](#predefined-ssl-policies) , obsahovat různé konfigurace protokolu SSL verze zásad a povolené šifrovací sady. Máte také možnost toodefine hello [vlastní zásady protokolu SSL](#configure-a-custom-ssl-policy) podle svých požadavků.

## <a name="get-available-ssl-options"></a>Získání dostupných možností protokolu SSL

Hello `Get-AzureRMApplicationGatewayAvailableSslOptions` rutiny poskytuje seznam dostupných předdefinovaných zásad, k dispozici šifrovací sady a verze protokolu, které lze konfigurovat. Hello následující příklad ukazuje příklad výstup z spuštění rutiny hello.
...
Aplikační brána se dodává s 3 předem definované zásady, které lze použít. Hello `Get-AzureRmApplicationGatewaySslPredefinedPolicy` rutina načte tyto zásady. Každá zásada má jiný protokol verze a sady šifer, které jsou povolené. Použití těchto předdefinovaných zásad tooquickly nakonfigurovat zásady protokolu SSL na aplikační brány. Ve výchozím nastavení **AppGwSslPolicy20170401** je vybraná, pokud je definována žádná konkrétní zásady protokolu SSL.

Hello tady je příklad spuštění `Get-AzureRmApplicationGatewaySslPredefinedPolicy`.
...
Následující ukázka Hello nastaví vlastní zásady protokolu SSL na aplikační brány. Nastaví verzi protokolu minimální hello příliš`TLSv1_1` a umožňuje hello následující šifrovací sady:

* TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384
* TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256

> [!IMPORTANT]
> Když nakonfigurujete vlastní zásady protokolu SSL, musí být vybrán alespoň jeden šifrovací sada z hello následující seznam. Aplikační brána používá RSA SHA256 šifrovací sady pro správu back-end.
> * TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256 
> * TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256
> * TLS_DHE_RSA_WITH_AES_128_GCM_SHA256
> * TLS_RSA_WITH_AES_128_GCM_SHA256
...
Hello následující příklad vytvoří novou aplikační bránu s předem definované zásady protokolu SSL.
...
Navštivte [přehled přesměrování Application Gateway](application-gateway-redirect-overview.md) toolearn jak tooredirect HTTP provozu tooa koncový bod HTTPS.
