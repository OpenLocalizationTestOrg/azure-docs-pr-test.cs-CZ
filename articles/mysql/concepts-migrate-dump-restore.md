...
-   Vyhněte se `single-transaction` možnost v mysqldump při vypsání velmi velké databáze. Vypsání mnoho tabulek v rámci jedné transakce způsobí dodatečné úložiště a prostředky paměti, který se má používat při obnovení a může způsobit zpoždění výkonu nebo omezení zdrojů.
-   Použití s více hodnotami vloží při načítání s SQL se minimalizuje režijní náklady na spuštění příkazu vypsání databáze. Při použití souborů výpisu paměti generované mysqldump nástroj, jsou ve výchozím nastavení povolené vloží s více hodnotami. 
