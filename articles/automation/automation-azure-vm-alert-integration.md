...
2. Vyberte jednu z vašich virtuálních počítačů.  Zobrazí se okno řídicího panelu virtuální počítač a **nastavení** okno záhlavím vpravo.  
3. Z **nastavení** okno, v části vyberte části monitorování **výstrah pravidla**.
4. Na **výstrah pravidla** okně klikněte na tlačítko **přidat upozornění**.

Otevře **přidání pravidla výstrahy** okno, kde můžete konfigurovat podmínky pro výstrahu a zvolit si takový jeden nebo všechny tyto možnosti: odeslání e-mailu, použijte webhook, jehož a předat výstrahy do jiného systému, popř. spuštěn Sada Automation runbook v odpovědi se pokusí opravit tento problém.

## <a name="configure-a-runbook"></a>Konfigurace sady runbook
Konfigurace sady runbook spouštět, když je dosaženo prahové hodnoty pro výstrahu virtuálního počítače, vyberte **sady Automation Runbook**. V **konfigurace runbook** okno, můžete vybrat spuštění sady runbook a účet Automation ke spuštění sady runbook.
...
Sady runbook, které jsou k dispozici ověřování v Azure pomocí objektu služby. Pokud zvolíte možnost spuštění sady runbook v jednom z vaší existující účty Automation, jsme automaticky vytvoří služba hlavní za vás. Pokud se rozhodnete vytvořit nový účet Automation, pak automaticky vytvoříme účet a objekt služby. V obou případech dva prostředky bude vytvořen také v účtu Automation – certifikát asset s názvem **AzureRunAsCertificate** a připojení asset s názvem **AzureRunAsConnection**. Sady runbook použije **AzureRunAsConnection** k ověření pomocí Azure, abyste mohli provádět akce správy u virtuálního počítače.
...
Po dokončení konfigurace se zobrazí název sady runbook se zobrazí v **přidání pravidla výstrahy** okno.

![Runbook nakonfigurovat](media/automation-azure-vm-alert-integration/RunbookConfigured.png)

Klikněte na tlačítko **OK** v **přidání pravidla výstrahy** okno a pravidlo výstrahy se vytvoří a aktivovat, pokud je virtuální počítač ve spuštěném stavu.
