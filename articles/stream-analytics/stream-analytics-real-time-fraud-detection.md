...
V tomto postupu nejprve vytvoříte na obor názvů centra událostí a pak přidáte do této namepsace centra událostí. Obory názvů centra událostí se používají k logickému seskupení souvisejících událostí sběrnice instancí. 

1. Přihlaste se k portálu Azure a klikněte na tlačítko **nový** > **Internet věcí** > **centra událostí**. 

2. V **vytvoření oboru názvů** okno, zadejte název oboru názvů, jako `<yourname>-eh-ns-demo`. Můžete použít libovolný název pro obor názvů, ale název musí být platné adresy URL a musí být jedinečný v Azure. 
...
Než začnete TelcoGenerator aplikace, musíte ho nakonfigurovat tak, aby se budou odesílat záznamy volání do centra událostí, který jste právě vytvořili.
...
3.  V `<appSettings>` elementu, to udělat:
...
1. Na řídicím panelu nebo **všechny prostředky** okno, vyhledejte a vyberte `sa_frauddetection_job_demo` úlohy služby Stream Analytics. 
2. V **úlohy topologie** části okno úlohy Stream Analytics, klikněte na tlačítko **vstup** pole.
...
3. Klikněte na tlačítko  **+ &nbsp;přidat** a potom vyplňte v okně s těmito hodnotami:
...
Velmi jednoduchý dotaz může být jednoduše načíst všechna příchozí data. Ale často vytvořit dotazy, které vypadají pro konkrétní data, nebo pro vztahy v datech. V této části kurzu vytvoříte a otestovat několik dotazů další několik způsobů, ve kterých můžete převést vstupní datový proud pro analýzu. 
...
2. Na portálu vraťte se k okno úloha streamování Analytics. (Pokud jste zavřeli okna, vyhledejte `sa_frauddetection_job_demo` v **všechny prostředky** okno.)
...
4. V **dotazu** okně klikněte na tlačítko se tečkami vedle `CallStream` vstup a pak vyberte **vzorová data ze vstupu**.
...
1. V portálu Azure vrátíte do okna úlohy streamování Analytics. (Pokud jste zavřeli okna, vyhledejte `sa_frauddetection_job_demo` v **všechny prostředky** okno.)
2. V **úlohy topologie** klikněte na položku **výstup** pole. 
3. V **výstupy** okně klikněte na tlačítko  **+ &nbsp;přidat** a potom vyplňte v okně s těmito hodnotami:
...
3. V **spuštění úlohy** okně pro výstup čas spuštění úlohy, vyberte **nyní**. 
...
1. Zastavte úlohu streamování Analytics. V **úlohy** okně klikněte na tlačítko **Zastavit** v horní části.
...
Můžete pokračovat v tomto kurzu v následujících článcích:
...
* [Tom, jak ukládat data z Azure Stream Analytics v Azure Redis Cache pomocí Azure Functions](stream-analytics-functions-redis.md). Tento článek ukazuje, jak používat Azure Functions k zápisu podvodné volání k Azure Redis cache prostřednictvím fronty Service Bus.
