---
title: "hierarchie aaaCountry nebo oblast se používá Azure Traffic Manager geografické typ směrování | Microsoft Docs"
description: "Tento článek obsahuje seznam země nebo oblast hierarchii používá Azure Traffic Manager geografické typ směrování"
services: traffic-manager
documentationcenter: 
author: kumudd
manager: timlt
editor: 
ms.assetid: 
ms.service: traffic-manager
ms.devlang: na
ms.topic: article
ms.tgt_pltfrm: na
ms.workload: infrastructure-services
ms.date: 03/22/2017
ms.author: kumud
ms.openlocfilehash: d09fc896951fa1086822d69316468e7a98861591
ms.sourcegitcommit: 523283cc1b3c37c428e77850964dc1c33742c5f0
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 10/06/2017
---
# <a name="countryregion-hierarchy-used-by-azure-traffic-manager-for-geographic-traffic-routing-method"></a>Pomocí Azure Traffic Manager používá pro metodu směrování provozu geografické hierarchie země nebo oblast

Tento článek obsahuje seznam hello zemí a oblastí, používá hello **Geographic** metoda směrování v Azure Traffic Manageru provozu. Můžete také získat tyto informace prostřednictvím kódu programu voláním [rozhraní API REST Azure Traffic Manager](https://docs.microsoft.com/rest/api/trafficmanager/). 

- World(World)

    - GEO-EU(Europe)

        - AD(Andorra)

        - Al(Albania)

        - AT(Austria)

        - AX (Ålandy)

        - BA (Bosny a Hercegoviny)

        - BE(Belgium)

        - BG(Bulgaria)

        - BY(Belarus)

        - CH(Switzerland)

        - CY(Cyprus)

        - CZ (Česká republika)

        - DE(Germany)

        - DK(Denmark)

        - EE(Estonia)

        - ES(Spain)

        - Fi(Finland)

        - FO (Faerské ostrovy)

        - FR(France)

        - GB (Česká republika)

            - GB-Eng(England)

            - GB-NIR(Northern Ireland)

            - GB-SCT(Scotland)

            - GB-WLS(Wales)

        - GG(Guernsey)

        - GI(Gibraltar)

        - GR(Greece)

        - HR(Croatia)

        - HU(Hungary)

        - IE(Ireland)

        - Zasílání rychlých zpráv (ostrov Man)

        - IS(Iceland)

        - IT(Italy)

        - JE(Jersey)

        - LI(Liechtenstein)

        - LT(Lithuania)

        - LU(Luxembourg)

        - LV(LATVIA)

        - MC(Monaco)

        - MD(Moldova)

        - ME(Montenegro)

        - MK (Makedonie, FYRO)

        - MT(Malta)

        - NL(Netherlands)

        - No(Norway)

        - PL(Poland)

        - PT(Portugal)

        - RO(ROMANIA)

        - RS(Serbia)

        - RU(Russia)

        - SE(Sweden)

        - SI(Slovenia)

        - Sj(Svalbard)

        - Sk(slovakia)

        - SM (SV)

        - UA(Ukraine)

        - VA (Vatikán)

        - XJ (Jan Mayen)

        - Xk(Kosovo)

    - Geograficky ME(Middle East)

        - AE (Spojené arabské emiráty)

        - BH(Bahrain)

        - IL(Israel)

        - IQ(Iraq)

        - IR(Iran)

        - Jo(Jordan)

        - KW(Kuwait)

        - LB(Lebanon)

        - OM(Oman)

        - PS (palestinskou)

        - QA(Qatar)

        - SY(Syria)

        - Přidružení zabezpečení (Saúdská Arábie)

        - TR(Turkey)

        - YE(Yemen)

    - Geograficky NA(North America / Central America / Caribbean)

        - AG (Antigua a Barbuda)

        - AI(Anguilla)

        - AW(Aruba)

        - BB(Barbados)

        - Blokovaných (Svatý Bartoloměj)

        - BM(Bermuda)

        - BQ(Bonaire)

        - BS(Bahamas)

        - BZ(Belize)

        - CA(Canada)

            - CA-AB(Alberta)

            - BC(British Columbia) certifikační Autority

            - CA-MB(Manitoba)

            - NB(New Brunswick) certifikační Autority

            - NL(Newfoundland and Labrador) certifikační Autority

            - NS(Nova Scotia) certifikační Autority

            - NT(Northwest Territories) certifikační Autority

            - CA-nu(Nunavut)

            - CA-on(Ontario)

            - PE(Prince Edward Island) certifikační Autority

            - CA-QC(Québec)

            - CA-sk(Saskatchewan)

            - YT(Yukon Territory) certifikační Autority

        - Znaky CR (Kostarika)

        - Cu(Cuba)

        - CW(CuraÃ§AO)

        - DM(Dominica)

        - Proveďte (Dominikánská republika)

        - GD(Grenada)

        - GL(Greenland)

        - GP(Guadeloupe)

        - GT(Guatemala)

        - HN(Honduras)

        - HT(Haiti)

        - JM(Jamaica)

        - KN (Svatý Kryštof a Nevis)

        - KY (Kajmanské ostrovy)

        - LC (Svatá Lucie)

        - MF (Svatý Martin)

        - MQ(Martinique)

        - MS(Montserrat)

        - MX(Mexico)

        - NI(Nicaragua)

        - Pa(Panama)

        - PM (Saint Pierre a Miquelon)

        - Kód PR (Portoriko)

        - SV (El Salvador)

        - SX (Sint Maarten)

        - TC (Turks a Caicos)

        - TT (Trinidad a Tobago)

        - ÁLNÍ (USA Odlehlé ostrovy)

        - US(United States)

            - US-ak(Alaska)

            - US-Al(Alabama)

            - US-ar(Arkansas)

            - US-az(Arizona)

            - US-CA(California)

            - US-co(Colorado)

            - US-CT(Connecticut)

            - DC(District of Columbia) USA

            - US-de(Delaware)

            - US-FL(Florida)

            - US-GA(Georgia)

            - US-Hi(hawaii)

            - US-IA(Iowa)

            - US-ID(Idaho)

            - US-IL(Illinois)

            - US-in(Indiana)

            - US-ks(Kansas)

            - US-ky(Kentucky)

            - US-la(Louisiana)

            - US-ma(Massachusetts)

            - US-MD(Maryland)

            - US-ME(Maine)

            - US-mi(Michigan)

            - US-Mn(Minnesota)

            - US-mo(Missouri)

            - US-MS(Mississippi)

            - US-MT(Montana)

            - NC(North Carolina) USA

            - ND(North Dakota) USA

            - US-ne(Nebraska)

            - NH(New Hampshire) USA

            - NJ(New Jersey) USA

            - NM(New Mexico) USA

            - US-NV(Nevada)

            - NY(New York) USA

            - US-OH(Ohio)

            - US-OK(Oklahoma)

            - US-OR(oregon)

            - US-Pa(Pennsylvania)

            - RI(Rhode Island) USA

            - SC(South Carolina) USA

            - SD(South Dakota) USA

            - US-TN(Tennessee)

            - US-TX(Texas)

            - US-ut(Utah)

            - US-va(Virginia)

            - US-VT(Vermont)

            - US-WA(Washington)

            - US-Wi(Wisconsin)

            - WV(West Virginia) USA

            - US-WY(Wyoming)

        - VC (Svatý Vincenc a Grenadiny hello)

        - VG (British Panenské ostrovy)

        - VI (USA Primárních ostrovy)

        - XE (Sint Eustach)

        - XS(Saba)

    - GEO-As(ASIA)

        - AF(Afghanistan)

        - AM(Armenia)

        - AZ(Azerbaijan)

        - BD(Bangladesh)

        - Bn(Brunei)

        - BT(Bhutan)

        - Kopie (Kokosové ostrovy)

        - CN(China)

        - CX (Vánocemi Island)

        - GE(Georgia)

        - (Hong Kong) (Hongkong – zvláštní správní oblast)

        - ID(Indonesia)

        - IN(India)

        - Vstupně-výstupní operace (britské Indickém oceánu území)

        - JP(Japan)

        - KG(Kyrgyzstan)

        - KH(Cambodia)

        - KP (Severní Korea)

        - KR(Korea)

        - KZ(Kazakhstan)

        - La(Laos)

        - LK (Srí Lanka)

        - MM(Myanmar)

        - Mn(Mongolia)

        - PŘE (zboží oblast ČLR)

        - MV(Maldives)

        - My(Malaysia)

        - NP(Nepal)

        - PH(Philippines)

        - PK(Pakistan)

        - SG(Singapore)

        - TH(Thailand)

        - TJ(Tajikistan)

        - TL(Timor_Leste)

        - TM(Turkmenistan)

        - Tw(Taiwan)

        - Uz(Uzbekistan)

        - Vn(Vietnam)

    - GEO-AF(Africa)

        - AO(Angola)

        - BF (Burkina FASSO)

        - BI(Burundi)

        - BJ(Benin)

        - BV (Bouvet Island)

        - BW(Botswana)

        - CD (Konžská demokratická republika)

        - CR (Středoafrická republika)

        - CG(Congo)

        - CI (Pobřeží slonoviny)

        - CM(Cameroon)

        - Odchylka nákladů (Cabo Verde)

        - DJ(Djibouti)

        - DZ(Algeria)

        - EG(Egypt)

        - ER(Eritrea)

        - ET(Ethiopia)

        - GA(Gabon)

        - GH(Ghana)

        - GM(Gambia)

        - GN(Guinea)

        - GQ (Rovníkové Guinejská)

        - GW(Guinea_Bissau)

        - KE(Kenya)

        - KM(Comoros)

        - LR(Liberia)

        - LS(Lesotho)

        - LY(Libya)

        - MA(Morocco)

        - MG(Madagascar)

        - ML(Mali)

        - MR(Mauritania)

        - MU(Mauritius)

        - MW(Malawi)

        - MZ(Mozambique)

        - NA(Namibia)

        - Ne(niger)

        - NG(Nigeria)

        - RE(Réunion)

        - RW(Rwanda)

        - SC(Seychelles)

        - SD(Sudan)

        - SH (SV. Helena, Ascensión a Tristan da Cunha)

        - SL (Sierry Leone)

        - SN(Senegal)

        - SO(Somalia)

        - SS (Jižní Súdánu)

        - ST (Svatý Tomáš a Princův ostrov)

        - SZ(Swaziland)

        - TD(CHAD)

        - TF (Francouzská jižní území)

        - TG(Togo)

        - TN(Tunisia)

        - TZ(Tanzania)

        - UG(Uganda)

        - YT(Mayotte)

        - ZA (Jižní Afrika)

        - Zm(Zambia)

        - ZW(Zimbabwe)

    - GEO-an(Antarctica)

        - AQ(Antarctica)

    - Geograficky SA(South America)

        - AR(Argentina)

        - BO(Bolivia)

        - BR(Brazil)

        - CL(Chile)

        - CO(Colombia)

        - EC(Ecuador)

        - Cizího klíče (Falklandy)

        - GF (francouzštině Guyana)

        - GS (Jižní Georgie a Jižní Sandwichovy ostrovy)

        - GY(Guyana)

        - PE(Peru)

        - PY(Paraguay)

        - SR(Suriname)

        - UY(Uruguay)

        - VE(Venezuela)

    - Geograficky AP(Australia / Pacific)

        - As(American Samoa)

        - AU(Australia)

            - AU ACT(Australian Capital Territory)

            - AU NSW(New South Wales)

            - AU NT(Northern Territory)

            - AU-QLD(Queensland)

            - AU SA(South Australia)

            - AU-TAS(Tasmania)

            - AU-Vic(Victoria)

            - AU WA(Western Australia)

        - CK (Cookovy ostrovy)

        - FJ(Fiji)

        - FM(Micronesia)

        - GU(Guam)

        - HM (Heardův ostrov a McDonaldovy ostrovy)

        - Ki(Kiribati)

        - MH (zařadit ostrovy)

        - MP (Severní Mariany)

        - NC (Nová Kaledonie)

        - NF (Ostrov Norfolk)

        - NR(Nauru)

        - NU(Niue)

        - NZ (Nový Zéland)

        - PF (francouzštině Polynésie)

        - PG (papujsko-Guinejská)

        - PN (Pitcairnovy ostrovy)

        - PW(Palau)

        - SB (Šalamounovy ostrovy)

        - TK(Tokelau)

        - TO(Tonga)

        - TV(Tuvalu)

        - VU(Vanuatu)

        - WF (Wallis a Futuna)

        - WS(Samoa)

## <a name="next-steps"></a>Další kroky

- Další informace o [metoda směrování v Azure Traffic Manageru provozu Geographic](traffic-manager-routing-methods.md#geographic).
