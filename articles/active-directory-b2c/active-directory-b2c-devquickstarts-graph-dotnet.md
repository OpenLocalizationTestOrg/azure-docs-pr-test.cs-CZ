...
---
# <a name="azure-ad-b2c-use-the-graph-api"></a>Azure AD B2C: Rozhraní Graph API pomocí
...
> [!IMPORTANT]
> Použít rozhraní Graph API pomocí svého klienta B2C, budete muset registraci vyhrazené aplikace pomocí Obecné *registrace aplikace* okno na portálu Azure **není** Azure AD B2C  *Aplikace* okno. Nelze znovu použít už existující B2C aplikace, které jste zaregistrovali v Azure AD B2C *aplikace* okno.
...
6. V okně nastavení klikněte na **klíče** a přidejte nový klíč (také označovaný jako sdílený tajný klíč klienta). Také zkopírujte jej pro použití v další části.
...
1. Pokračování v okně registrace aplikace portálu Azure, vyberte svou aplikaci.
2. V okně nastavení klikněte na **požadovaná oprávnění**.
3. V okně potřebná oprávnění, klikněte na **Windows Azure Active Directory**.
4. V okně povolit přístup, vyberte **pro čtení a zápis dat adresáře** oprávnění z **oprávnění aplikací** a klikněte na tlačítko **Uložit**.
5. Nakonec zpět v okně potřebná oprávnění, klikněte na **udělit oprávnění** tlačítko.
...
Nejprve stáhnout a nainstalovat [Microsoft Online Services Sign-In Assistant](http://go.microsoft.com/fwlink/?LinkID=286152). Potom stáhněte a nainstalujte [64-bit modulu Azure Active Directory pro prostředí Windows PowerShell](http://go.microsoft.com/fwlink/p/?linkid=236297).

Po instalaci modulu prostředí PowerShell, otevřete prostředí PowerShell a připojte se do svého klienta B2C. Po spuštění `Get-Credential`, zobrazí se výzva pro zadání uživatelského jména a hesla, zadejte uživatelské jméno a heslo účtu správce klienta B2C.
