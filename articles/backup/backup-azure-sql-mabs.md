...
---
# <a name="back-up-sql-server-tooazure-with-azure-backup-server"></a>Zálohování serveru SQL Server tooAzure pomocí serveru Azure Backup
Tento článek vás provede kroky konfigurace hello pro zálohování databáze SQL serveru pomocí Microsoft Azure Backup Server (MABS).
...
Než začnete, ujistěte se, že máte [nainstalované a připravené hello serveru Azure Backup](backup-azure-microsoft-azure-backup.md).
...
1. Na hello uživatelské rozhraní serveru služby zálohování Azure, klikněte na tlačítko hello **ochrany** pracovního prostoru.
...
3. MABS zobrazí úvodní obrazovka start s hello pokyny o vytváření **skupiny ochrany**. Klikněte na **Další**.
4. Vyberte **servery**.

    ![Vybrat typ skupiny ochrany – 'servery.](./media/backup-azure-backup-sql/pg-servers.png)
5. Rozbalte počítači serveru SQL Server hello, kde jsou přítomny toobe databáze hello zálohovat. MABS zobrazí různé zdroje dat, které lze zálohovat z tohoto serveru. Rozbalte hello **všechny sdílené složky SQL** a vyberte hello databáze (v tomto případě jsme vybrali ReportServer$ MSDPM2012 a ReportServer$ MSDPM2012TempDB) toobe zálohovat. Klikněte na **Další**.
...
8. Klikněte na **Další**
    ...
    Zobrazuje MABS hello celkového prostoru úložiště k dispozici a využití místa na disku potenciální hello.

    ![Přidělení disku](./media/backup-azure-backup-sql/pg-storage.png)

    Ve výchozím nastavení vytvoří MABS jeden svazek na zdroj dat (databázi systému SQL Server), který se používá pro hello kopie prvotní zálohy. Tento přístup hello Správce logických disků (LDM) omezuje MABS ochrany too300 datové zdroje (databáze systému SQL Server). toowork obejít toto omezení, vyberte hello **společně umísťovat data do fondu úložiště DPM**, možnost. Pokud použijete tuto možnost, MABS používá na jednom svazku pro více zdrojů dat, která umožňuje MABS tooprotect až too2000 databází SQL.

    Pokud **automaticky rozšiřovat svazky hello** je vybraná možnost, MABS můžete účet pro hello vyšší zálohování svazku s růstem hello provozními daty. Pokud **automaticky rozšiřovat svazky hello** možnost není vybraná, MABS omezuje hello používá úložiště záloh toohello zdroje dat ve skupině ochrany hello.
...
10. Vyberte, když chcete toorun Kontrola konzistence hello a klikněte na tlačítko **Další**.
    ...
    MABS můžete provést konzistence kontrola toocheck hello integrity hello bodu zálohy. Vypočte kontrolního součtu hello hello záložního souboru na provozním serveru hello (počítači serveru SQL Server v tomto scénáři) a hello zálohovaná data pro tento soubor na MABS. V případě hello jsou v konfliktu se předpokládá, že hello soubor zálohy v MABS je poškozený. MABS rectifies hello zálohovaná data odesláním hello bloky odpovídající neshoda toohello kontrolního součtu. Kontrola konzistence hello je operace náročné na výkon, Správci mají možnost hello plánovaná kontrola konzistence hello nebo spuštěním automaticky.
