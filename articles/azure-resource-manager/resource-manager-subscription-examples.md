...
| Skupina prostředků |rgBitBucket |Obsahuje aplikace webového serveru a databázového serveru |
| Skupina prostředků |rgCoreNetworks |Obsahuje virtuální sítě a připojení brány site-to-site |
...
| --- | --- |
| ApplicationOwner |Název osobě, která spravuje této aplikace. |
...
| --- | --- | --- |
| Virtual Network |vnInternal |Použít s aplikací BitBucket a je připojená přes ExpressRoute k podnikové síti společnosti Contoso.  Podsíť (sbBitBucket) poskytuje aplikaci s konkrétní prostor IP adres. |
| Virtual Network |vnExternal |Chcete-li k dispozici pro budoucí aplikace, které vyžadují veřejné koncové body. |
| Skupina zabezpečení sítě |nsgBitBucket |Zajišťuje, že prostor pro útoky této úlohy je minimalizován tím, že se připojení pouze na portu 443 pro podsíť tam, kde aplikace platný (sbBitBucket). |
...
| --- | --- | --- |
| **CanNotDelete** |vnInternal |Zabrání uživatelům odstranit virtuální sítě nebo podsítě, ale nezabrání přidání nových podsítí. |
...
| Pole | Efekt | Popis |
| --- | --- | --- |
| location |Odepřít |Odepřete vytvoření všechny prostředky mimo USA datových centrech. |
| tags |Odepřít |Vyžadují aplikace vlastníka značky |
| tags |Odepřít |Vyžadovat oddělení značky. |
| tags |Připojit |Značka připojí ke každé skupině prostředků, který určuje produkční prostředí. |
...
| Typ prostředku | Name (Název) | Popis |
| --- | --- | --- |
| Virtual Network |vnInternal |Slouží Contoso věrného karty vývojového prostředí a je připojená přes ExpressRoute k podnikové síti společnosti Contoso. |

Pro **produkční předplatné**, vytvoří:

| Typ prostředku | Name (Název) | Popis |
| --- | --- | --- |
| Virtual Network |vnExternal |Hostuje aplikaci věrného karta a není připojené přímo k ExpressRoute společnosti Contoso. Kód se instaluje prostřednictvím jejich systému zdrojového kódu přímo do služby PaaS. |
| Skupina zabezpečení sítě |nsgBitBucket |Zajišťuje, že prostor pro útoky této úlohy je minimalizován povolením jenom v vázané na komunikaci na portu TCP 443.  Contoso je také příčin pro další ochranu pomocí brány Firewall webových aplikací. |
...
| Typ zámku | Prostředek | Popis |
| --- | --- | --- |
| **CanNotDelete** |vnExternal |Chcete-li zabránit neoprávněným osobám v odstranění virtuální sítě nebo podsítě. Zámek nezabrání přidání nových podsítí. |
