...
V tomto kurzu hello aplikace, které jste vytvořili v [webové aplikace Node.js pomocí Express] kurzu je rozšířeno pomocí hello Microsoft Azure klientské knihovny pro Node.js toowork se službami pro správu dat. Vytvoření aplikace založené na webu – seznam úloh, můžete nasadit tooAzure můžete rozšířit vaší aplikace. Seznam úkolů Hello umožňuje uživateli načíst úlohy, přidat nové úkoly a úkoly označit jako dokončená.

položky úkolů Hello jsou uložené ve službě Azure Storage. Úložiště Azure poskytuje úložiště nestrukturovaných dat, které je odolné proti chybám a vysoce dostupné. Úložiště Azure obsahuje několik datových struktur, kde můžete ukládat a přistupovat k datům. Můžete využívat hello úložiště služby hello rozhraní API, které jsou součástí hello Azure SDK pro Node.js nebo přes rozhraní REST API. Další informace najdete v tématu [ukládání a přístup k datům v Azure].
...
Obsahuje hello následující informace:
...
Musíte zadat v tooaccess přihlašovací údaje úložiště Azure Storage. To se provádí s využitím nastavení aplikace v souboru web.config hello.
nastavení v souboru web.config Hello jsou předány jako tooNode proměnné prostředí, které jsou pak přečte hello Azure SDK.

> [!NOTE]
> Přihlašovací údaje úložiště používají jenom při hello aplikace je nasazená tooAzure. Při spuštění v emulátoru hello, aplikace hello používá emulátor úložiště hello.
>
>
...
3. Z hello okno Azure Powershell zadejte následující rutinu informace o účtu úložiště tooretrieve hello hello:
   ...
   Hello Předchozí rutina načte seznam hello účty úložiště a klíče účtu, které jsou přidružené k vaší hostované služby.
...
V této části hello základní aplikace vytvořené v hello **express** příkaz je rozšířit přidáním **task.js** obsahující hello model pro úkoly. Upravit existující hello **app.js** souboru a vytvořte novou **tasklist.js** soubor, který používá hello model.

### <a name="create-hello-model"></a>Vytvoření modelu hello
1. V hello **WebRole1** adresáře, vytvořte nový adresář s názvem **modely**.
2. V hello **modely** adresáře, vytvořte nový soubor s názvem **task.js**. Tento soubor obsahuje hello model pro úkoly hello vytvořené v aplikaci.
...
4. V dalším kroku přidejte kód toodefine a export objektu Task hello. objekt úlohy Hello je zodpovědná za připojení toohello tabulky.
...
2. Přidejte následující kód příliš hello**tasklist.js**. Tento kód načte hello azure a asynchronních moduly, které jsou používány **tasklist.js** a definuje hello **TaskList** funkce, která se předá instance hello **úloh** jsme objektu definovaného dříve:
...
3. V souboru app.js hello, posuňte se dolů toowhere zobrazí hello následující řádek:
    ...
    Nahraďte hello předcházející řádky s hello následující kód. Tento kód inicializuje instanci <strong>úloh</strong> s účtem úložiště tooyour připojení. Hello <strong>úloh</strong> je předán toohello <strong>TaskList</strong>, který používá ho toocommunicate s služby Table hello:
...
2. Nahraďte obsah hello hello **index.jade** soubor s hello následující kód. Tento kód definuje hello zobrazení pro zobrazení stávající úlohy a definuje formuláře pro přidání nové úlohy a označit stávajících jako dokončenou.
...
Hello **layout.jade** souboru v hello **zobrazení** directory se používá jako globální šablona pro ostatní **.jade** soubory. V tomto kroku změnit hello **layout.jade** souboru toouse [Twitter Bootstrap](https://github.com/twbs/bootstrap), což je sada nástrojů, který umožňuje snadno toodesign pěkných webů.
...
2. Z hello **zobrazení** složku, otevřete hello **layout.jade** soubor v textovém editoru a nahraďte obsah hello hello následující:
...
Otevře se prohlížeč Hello a zobrazí hello následující stránky:
...
Zadáním hello **– spusťte** možnost v předchozí rutiny hello hello prohlížeč otevře a zobrazí vaši aplikaci běžící v Azure po dokončení publikování.

![Okno prohlížeče zobrazující stránku hello Moje seznamu úkolů. Hello URL značí, že stránku hello je teď hostované v Azure.](./media/table-storage-cloud-service-nodejs/getting-started-1.png)
