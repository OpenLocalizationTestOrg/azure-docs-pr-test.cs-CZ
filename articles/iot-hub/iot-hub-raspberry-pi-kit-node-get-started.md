...
1. Otevřete konfigurační soubor spuštěním následujících příkazů:
   ...
   Existují dvě položky v tomto souboru můžete configurate. První z nich je `interval`, která definuje časový interval (v milisekundách) mezi dvě zprávy, které odesílají do cloudu. Druhý `simulatedData`, což je logickou hodnotu pro jestli se má používat data simulované snímačů, nebo ne.
