...
---
# <a name="connect-intel-edison-tooazure-iot-hub-nodejs"></a>Připojit Intel Edison tooAzure IoT Hub (Node.js)
...
3. Pak přejděte toohello úložišti složky toorun hello následující příkaz tooinstall všechny balíčky, může trvat toocomplete několika minut.
...
### <a name="configure-and-run-hello-sample-application"></a>Nakonfigurujte a spusťte ukázkové aplikace hello
...
1. Otevřete konfigurační soubor hello spuštěním hello následující příkazy:
   ...
   Existují dvě makra v tomto souboru můžete configurate. Hello nejprve jeden je `INTERVAL`, která definuje hello časový interval mezi dvě zprávy, které odesílají toocloud. Hello druhá `simulatedData`, což je logickou hodnotu pro jestli toouse simulated data snímačů, nebo ne.
