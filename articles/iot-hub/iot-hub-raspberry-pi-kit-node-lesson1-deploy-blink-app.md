...
Naklonujte ukázkovou aplikaci Node.js z Githubu a pomocí nástroje gulp nasadit ukázkovou aplikaci pro váš malin pí 3. Ukázkovou aplikaci bliká DIODU připojené k panelu každé dvě sekundy. Pokud máte potíže, vyhledejte řešení na [řešení potíží s stránky](iot-hub-raspberry-pi-kit-node-troubleshooting.md).
...
Chcete-li spustit ukázkový kód, postupujte takto:
...
`app.js` v soubor `app` podsložky je klíče zdrojového souboru, který obsahuje kód, který ovládacího prvku Indikátor.
...
Nainstalujte Node.js a NPM pí spuštěním následujícího příkazu:

```bash
gulp install-tools
```

Tato úloha může trvat 10 minut na dokončení při prvním spuštění.
...
Teď byste měli vidět Indikátor na platformy blikat každé dvě sekundy.  Pokud nevidíte DIODU blikat, najdete v článku [Průvodce odstraňováním potíží s](iot-hub-raspberry-pi-kit-node-troubleshooting.md) pro řešení běžných potíží.
...
[Získat nástroje Azure](iot-hub-raspberry-pi-kit-node-lesson2-get-azure-tools-win32.md)
