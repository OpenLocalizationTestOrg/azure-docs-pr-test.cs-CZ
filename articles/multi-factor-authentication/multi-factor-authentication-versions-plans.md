...
Pokud jde o ochranu svých účtů, musí být dvoustupňové ověření standardní celé organizaci. Tato funkce je obzvláště důležité pro účty pro správu, které mají privilegovaný přístup k prostředkům. Z tohoto důvodu společnost Microsoft nabízí základní dvoustupňové ověření funkce správcům Office 365 a Azure. Pokud chcete upgradovat funkce pro vaši správci nebo rozšířit dvoustupňové ověřování s ostatními uživatelů, si můžete zakoupit Azure Multi-Factor Authentication. 

Tento článek se zabývá najdete vysvětlení rozdílu mezi verzemi, které nabízí pro správce a na plnou verzi Azure MFA a určuje funkce, které jsou v nich dostupné. Pokud jste připravení nasadit kompletní Azure MFA nabídka, v dalších částech popisuje možnosti implementace a jak Microsoft vypočítá spotřeby.
...
| Azure Multi-Factor Authentication | Často označuje jako "úplná" verze, Azure Multi-Factor Authentication nabízí nejkomplexnější sadu funkcí. Poskytuje další možnosti konfigurace prostřednictvím [portál Azure classic](https://manage.windowsazure.com), pokročilé vytváření sestav a podpora pro řadu místní i cloudové aplikace. Azure Multi-Factor Authentication je součástí Azure Active Directory Premium (plány P1 a P2) a Enterprise Mobility + Security (plány E3 a E5) a může být buď nasazené [v cloudu nebo místně](multi-factor-authentication-get-started.md). |
...
Nákup licencí, které zahrnují Azure Multi-Factor Authentication, jako je Azure Active Directory Premium (P1 a P2) nebo Enterprise Mobility + Security (E3 nebo E5) a přiřadit jim uživatelům v Azure Active Directory. 
...
2. **Za ověření** – podnikům, které chcete zapnout dvoustupňové ověřování pro velkou skupinu uživatelů, kteří potřebují zřídka ověřování. Fakturace je založen na řadě dvoustupňové ověření požadavků přijatých cloudové služby Azure MFA, bez ohledu na to, jestli tyto ověření úspěšné nebo byl odepřen. Tato fakturace se zobrazí na vaší příkaz Azure využití v balíčky 10 ověřování a hlásí systém Commerce denně. 
