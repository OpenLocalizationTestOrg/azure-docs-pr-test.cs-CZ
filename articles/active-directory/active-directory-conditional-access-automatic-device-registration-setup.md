...
---
# <a name="how-tooconfigure-automatic-registration-of-windows-domain-joined-devices-with-azure-active-directory"></a>Jak tooconfigure automatické registrace Windows připojených k doméně zařízení s Azure Active Directory

toouse [podmíněného přístupu na základě zařízení služby Azure Active Directory](active-directory-conditional-access-azure-portal.md), počítače musí být zaregistrován u služby Azure Active Directory (Azure AD). Seznam registrovaných zařízení ve vaší organizaci můžete získat pomocí hello [Get-MsolDevice](https://docs.microsoft.com/powershell/msonline/v1/get-msoldevice) rutiny v hello [modul Azure Active Directory PowerShell](/powershell/azure/install-msonlinev1?view=azureadps-2.0). 

Tento článek poskytuje hello kroky pro konfiguraci hello Automatická registrace zařízení se systémem Windows připojených k doméně se službou Azure AD ve vaší organizaci.
...
- Zařízení s Windows 10 v síti na pracovišti hello a hello rozšířené možnosti po zaregistrování s Azure AD, najdete v části [Windows 10 pro podnik hello: použití zařízení při práci](active-directory-azureadjoin-windows10-devices-overview.md).
- Windows 10 Enterprise E3 v CSP, najdete v části hello [Windows 10 Enterprise E3 v přehledu CSP](https://docs.microsoft.com/en-us/windows/deployment/windows-10-enterprise-e3-overview).


## <a name="before-you-begin"></a>Než začnete

Před zahájením konfigurace hello Automatická registrace zařízení připojených k doméně se systémem Windows ve vašem prostředí, by měl Seznamte se s hello Podporované scénáře a omezení hello.  
...
Než začnete, povolení hello automatickou registraci zařízení ve vaší organizaci, připojený k doméně, je nutné toomake, zda jsou spuštěny na aktuální verzi Azure AD connect.
...
Toto téma obsahuje hello požadované kroky pro všechny scénáře obvyklá konfigurace.  
...
Když jste dokončili postup hello vyžaduje, připojený k doméně zařízení jsou připravená tooautomatically registrace s Azure AD. Všechny připojené k doméně zařízení se systémem Windows 10 Anniversary Update a Windows Server 2016 automaticky zaregistrovat s Azure AD v zařízení restartujte nebo přihlášení uživatele. S Azure AD při restartování zařízení hello po dokončení operace připojení hello zaregistrovat nová zařízení.

Zařízení, které byly dříve připojené k síti na pracovišti tooAzure AD (například pro Intune) přechod příliš"*připojený k doméně, zaregistrováno v AAD*"; ale trvá delší dobu pro tento proces toocomplete ve všech zařízeních kvůli toohello normální tok doména a uživatelské aktivity.
...
- Windows 10. listopadu 2015 Update automaticky zaregistruje se s Azure AD **pouze** Pokud hello zavedení nastavení objektu zásad skupiny.

- toorollout hello automatické registrace počítače se systémem Windows nižší úrovně, můžete nasadit [balíček služby Windows Installer](#windows-installer-packages-for-non-windows-10-computers) toocomputers, kterou jste vybrali.

- Pokud nabízené hello zásad skupiny objekt tooWindows 8.1 připojená k doméně, bude proveden pokus o registraci; nedoporučujeme ale, že používáte hello [balíček služby Windows Installer](#windows-installer-packages-for-non-windows-10-computers) tooregister všechna zařízení Windows nižší úrovně. 

### <a name="create-a-group-policy-object"></a>Vytvoření objektu zásad skupiny 

zavedení hello toocontrol automatické registrace aktuální počítačů se systémem Windows, měli byste nasadit hello **registraci připojený k doméně počítače jako zařízení** zařízení toohello objekt zásad skupiny chcete tooregister. Například můžete nasadit hello zásad tooan organizační jednotku nebo skupinu zabezpečení tooa.
...
4. Zadejte název objektu zásad skupiny. Například *Automatická registrace tooAzure AD*. Vyberte **OK**.
5. Klikněte pravým tlačítkem na váš nový objekt zásad skupiny a potom vyberte **upravit**.
6. Přejděte příliš**konfigurace počítače** > **zásady** > **šablony pro správu** > **Windows Součásti** > **registrace zařízení**. Klikněte pravým tlačítkem na **registraci připojený k doméně počítače jako zařízení**a potom vyberte **upravit**.
...
tooregister připojený k doméně nižší úrovně počítače se systémem Windows ve federovaném prostředí, můžete stáhnout a nainstalovat tyto balíček Instalační služby systému Windows (.msi) ze služby Stažení softwaru na hello [připojení k pracovní ploše Microsoft pro počítače s Windows 10](https://www.microsoft.com/en-us/download/details.aspx?id=53554) stránky.
...
Instalační program Hello vytvoří naplánovanou úlohu na hello systému, který běží v kontextu uživatele hello. Při přihlášení uživatele hello tooWindows, aktivuje se úloha Hello. Úloha Hello bezobslužně zaregistruje hello zařízení s Azure AD s pověřeními uživatele hello po ověřování pomocí integrovaného ověřování systému Windows. toosee hello naplánované úlohy, v zařízení hello přejděte příliš**Microsoft** > **k síti na pracovišti**, a potom přejděte toohello Knihovna plánovače úloh.

## <a name="step-5-verify-registered-devices"></a>Krok 5: Ověření registrovaná zařízení

Úspěšně registrovaná zařízení ve vaší organizaci můžete zkontrolovat pomocí hello [Get-MsolDevice](https://docs.microsoft.com/powershell/msonline/v1/get-msoldevice) rutiny v hello [modul Azure Active Directory PowerShell](/powershell/azure/install-msonlinev1?view=azureadps-2.0).

Hello výstup této rutiny zobrazuje zařízení zaregistrovaná ve službě Azure AD. tooget všechna zařízení, použijte hello **– všechny** parametr a potom filtru je pomocí hello **deviceTrustType** vlastnost. Připojené k doméně zařízení mít hodnotu **připojený k doméně**.
...
* [Řešení potíží s automatickou registraci domény připojené počítače tooAzure AD – Windows 10](active-directory-device-registration-troubleshoot-windows-legacy.md)
