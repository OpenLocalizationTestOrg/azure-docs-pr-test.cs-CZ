...
---
# <a name="how-to-configure-automatic-registration-of-windows-domain-joined-devices-with-azure-active-directory"></a>Postup konfigurace automatické registrace zařízení se systémem Windows připojených k doméně se službou Azure Active Directory

Chcete-li použít [podmíněného přístupu na základě zařízení služby Azure Active Directory](active-directory-conditional-access-azure-portal.md), počítače musí být zaregistrován u služby Azure Active Directory (Azure AD). Seznam registrovaných zařízení ve vaší organizaci můžete získat pomocí [Get-MsolDevice](https://docs.microsoft.com/powershell/msonline/v1/get-msoldevice) rutiny v [modul Azure Active Directory PowerShell](/powershell/azure/install-msonlinev1?view=azureadps-2.0). 

Tento článek poskytuje kroky pro konfiguraci Automatická registrace zařízení se systémem Windows připojených k doméně se službou Azure AD ve vaší organizaci.


Další informace o:

- Podmíněný přístup, najdete v části [podmíněného přístupu na základě zařízení služby Azure Active Directory](active-directory-conditional-access-azure-portal.md). 
- Zařízení s Windows 10 na pracovišti a vylepšené prostředí při registraci s Azure AD, najdete v části [Windows 10 pro podnik: použití zařízení při práci](active-directory-azureadjoin-windows10-devices-overview.md).
- Windows 10 Enterprise E3 v CSP, najdete v článku [Windows 10 Enterprise E3 v přehledu CSP](https://docs.microsoft.com/en-us/windows/deployment/windows-10-enterprise-e3-overview).


## <a name="before-you-begin"></a>Než začnete

Před zahájením konfigurace automatické registrace zařízení připojených k doméně se systémem Windows ve vašem prostředí, by měl Seznamte se s Podporované scénáře a omezení.  
...
Než začnete, povolení automatické registrace zařízení ve vaší organizaci, připojený k doméně, musíte zajistit, že používáte aktuální verzi Azure AD connect.
...
Toto téma obsahuje požadované kroky pro všechny scénáře obvyklá konfigurace.  
...
| Krok 5: Ověření registrovaná zařízení          | ![Zaškrtnout][1]                            | ![Zaškrtnout][1]                    | ![Zaškrtnout][1]        |
...
Po dokončení požadovaných kroků zařízení připojených k doméně jsou automaticky registrace u služby Azure AD. Všechny připojené k doméně zařízení se systémem Windows 10 Anniversary Update a Windows Server 2016 automaticky zaregistrovat s Azure AD v zařízení restartujte nebo přihlášení uživatele. Zaregistrovat nová zařízení s Azure AD při restartování zařízení po dokončení operace připojení k doméně.

Zařízení, která byla dříve pracovišti přecházení mezi Azure AD (například pro Intune) "*připojený k doméně, zaregistrováno v AAD*"; ale pro tento proces dokončit ve všech zařízeních z důvodu normálního toku doména a uživatelské aktivity chvíli trvat.
...
- Windows 10. listopadu 2015 Update automaticky zaregistruje se s Azure AD **pouze** nastaveného zavedení objekt zásad skupiny.

- K zavedení automatické registrace počítače se systémem Windows nižší úrovně, můžete nasadit [balíček služby Windows Installer](#windows-installer-packages-for-non-windows-10-computers) do počítačů, které jste vybrali.

- Pokud nabízené objekt zásad skupiny do zařízení Windows 8.1 připojený k doméně, bude proveden pokus o registraci; nedoporučujeme ale, že používáte [balíček služby Windows Installer](#windows-installer-packages-for-non-windows-10-computers) k registraci všech zařízení Windows nižší úrovně. 

### <a name="create-a-group-policy-object"></a>Vytvoření objektu zásad skupiny 

Ovládejte automatické registrace aktuální počítačů se systémem Windows, měli byste nasadit **registraci připojený k doméně počítače jako zařízení** objekt zásad skupiny zařízení, který chcete zaregistrovat. Například můžete nasadit zásady do organizační jednotky nebo do skupiny zabezpečení.
...
4. Zadejte název objektu zásad skupiny. Například *Automatická registrace do služby Azure AD*. Vyberte **OK**.
5. Klikněte pravým tlačítkem na váš nový objekt zásad skupiny a potom vyberte **upravit**.
6. Přejděte na **konfigurace počítače** > **zásady** > **šablony pro správu** > **součásti systému Windows** > **registrace zařízení**. Klikněte pravým tlačítkem na **registraci připojený k doméně počítače jako zařízení**a potom vyberte **upravit**.
...
7. Vyberte **povoleno**a potom vyberte **použít**.
8. Vyberte **OK**.
9. Propojte objekt zásad skupiny k umístění podle vaší volby. Například můžete propojit k určité organizační jednotce. Můžete také může ho propojit s konkrétní skupiny zabezpečení počítačů, které automatickou registraci v rámci Azure AD. Nastavení této zásady pro všechny připojené k doméně Windows 10 a Windows Server 2016 počítače ve vaší organizaci, propojte objekt zásad skupiny do domény.
...
Ve federovaném prostředí zaregistrovat připojený k doméně počítače se systémem Windows nižší úrovně, můžete stáhnout a nainstalovat tyto balíček Instalační služby systému Windows (.msi) ze stažení softwaru společnosti Microsoft [připojení k pracovní ploše Microsoft pro počítače s Windows 10](https://www.microsoft.com/en-us/download/details.aspx?id=53554) stránky.
...
Instalační program vytvoří naplánovanou úlohu na systém, který běží v kontextu uživatele. Úloha se aktivuje, když se uživatel přihlásí systému Windows. Úloha bezobslužně zaregistruje zařízení s Azure AD s pověřeními uživatele po ověřování pomocí integrovaného ověřování systému Windows. Chcete-li zobrazit naplánované úlohy, v zařízení, přejděte na **Microsoft** > **k síti na pracovišti**a potom přejděte na Knihovna plánovače úloh.

## <a name="step-5-verify-registered-devices"></a>Krok 5: Ověření registrovaná zařízení

Úspěšně registrovaná zařízení ve vaší organizaci můžete zkontrolovat pomocí [Get-MsolDevice](https://docs.microsoft.com/powershell/msonline/v1/get-msoldevice) rutiny v [modul Azure Active Directory PowerShell](/powershell/azure/install-msonlinev1?view=azureadps-2.0).

Výstup této rutiny zobrazuje zařízení zaregistrovaná ve službě Azure AD. Všechna zařízení, použijte **– všechny** parametr a pak je filtrování pomocí **deviceTrustType** vlastnost. Připojené k doméně zařízení mít hodnotu **připojený k doméně**.

## <a name="next-steps"></a>Další kroky

* [Automatická registrace zařízení – nejčastější dotazy](active-directory-device-registration-faq.md)
* [Řešení potíží s automatickou registraci domény k počítačům připojeným k Azure AD – Windows 10 a Windows Server 2016](active-directory-device-registration-troubleshoot-windows.md)
* [Řešení potíží s automatickou registraci domény k počítačům připojeným k Azure AD – Windows 10](active-directory-device-registration-troubleshoot-windows-legacy.md)
