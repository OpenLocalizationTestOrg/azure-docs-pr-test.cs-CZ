...
Použijte samostatné správci toodesignate Azure Active Directory (Azure AD) pro různé funkce. Tyto správce můžete přístup k vybrané funkce v hello portál Azure nebo portál Azure classic a v závislosti na jejich role, bude být schopný toocreate nebo úprava uživatelů, přiřadit tooothers role pro správu, resetovat hesla uživatelů, správě uživatelských licencí a spravovat domény, mimo jiné. Uživatel, který je přiřazen roli správce bude mít hello stejná oprávnění napříč všemi hello cloudové služby toowhich vaše organizace předplatila, bez ohledu na to, jestli přiřadíte roli hello portálu hello Office 365, nebo v hello portál Azure classic nebo pomocí modul Hello Azure AD pro Microsoft PowerShell.

> [!IMPORTANT]
> Společnost Microsoft doporučuje, která můžete spravovat Azure AD pomocí hello [centra pro správu Azure AD](https://aad.portal.azure.com) v hello hello portál Azure místo použití portálu Azure classic, kterou se odkazuje v tomto článku. Jak tooassign rolí správce ve službě Azure AD Dobrý den, správce center, najdete v části [přiřazení rolí správce v Azure Active Directory](active-directory-assign-admin-roles-azure-portal.md).
...
* **Správce podmíněného přístupu**: uživatelé s touto rolí mají hello možnost toomanage nastavení podmíněného přístupu pro Azure Active Directory.
...
jak zjistit, tooassign správní role tooa uživatele v Azure Active Directory, toolearn [přiřadit uživatele tooadministrator role v Azure Active Directory](active-directory-users-assign-role-azure-portal.md).
