...
>[!NOTE]
> V každé doménové struktuře AD synchronizovat do Azure AD (přes Azure AD Connect) a pro uživatele, jehož chcete bezproblémové jednotné přihlašování se vytvoří účet počítače a SPN protokolu Kerberos. Přesunout `AZUREADSSOACCT` účet počítače pro organizaci jednotce (OU) ukládat další účty počítačů zajistit, že je spravovat stejným způsobem a není odstraněn.

>[!IMPORTANT]
>Důrazně doporučujeme, aby vám [mění dešifrovací klíč protokolu Kerberos](active-directory-aadconnect-sso-faq.md#how-can-i-roll-over-the-kerberos-decryption-key-of-the-azureadssoacct-computer-account) z `AZUREADSSOACCT` účet počítače minimálně každých 30 dnů.
...
5. Prohlížeč, pak vyžádá lístek ze služby Active Directory pro `AZUREADSSOACCT` účet počítače (což představuje Azure AD).
