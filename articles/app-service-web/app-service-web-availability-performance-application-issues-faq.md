...
Pokud chcete zjistit, zda používáte místní mezipaměti, zkontrolovat aplikační služby **nastavení aplikace** kartě. Pokud se používá místní mezipaměti, aplikace, nastavení `WEBSITE_LOCAL_CACHE_OPTION` je nastaven na `Always`. Další informace o místní mezipaměti najdete v tématu [místní mezipaměti služby aplikace přehled](https://docs.microsoft.com/azure/app-service/app-service-local-cache).
