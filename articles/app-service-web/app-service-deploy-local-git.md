...
V tomto kurzu se dozvíte, jak nasadit aplikaci, kterou chcete [Azure App Service] z úložiště Git v místním počítači. App Service podporuje tento přístup s **místní Git** v možnost nasazení [portálu Azure].  
Mnoho příkazů Git popsané v tomto článku se provádí automaticky při vytváření aplikace služby App Service pomocí [rozhraní příkazového řádku Azure] popsané [zde](app-service-web-get-started.md).
