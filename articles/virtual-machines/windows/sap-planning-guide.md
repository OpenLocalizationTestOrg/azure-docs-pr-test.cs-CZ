...
Cloud Computing je často používaný termín, který je získání více význam v rámci hello IT odvětví, z malých společností až toolarge a mezinárodních společnosti.
...
Pomocí služeb Microsoft Azure pro virtuální počítače nabízí společnost Microsoft komplexní platformu infrastruktury jako služby (IaaS). Služba Azure Virtual Machines teď v rámci IaaS podporuje aplikace využívající SAP NetWeaver. Tento dokument White Paper se popisují, jak tooplan a implementujte SAP NetWeaver založených na aplikace v rámci Microsoft Azure jako platformu hello výběru.

Hello dokumentu samotné se soustředí na dva hlavní aspekty:

* první část Hello popíše dva vzory podporované nasazení SAP NetWeaver na základě aplikací na platformě Azure. Také popíše obecné zpracování Azure s nasazení SAP v paměti.
* Druhá část Hello podrobně implementuje dvě různé scénáře hello popsané v první části hello.
...
* Systém SAP: kombinace hello databázového systému vrstvy a aplikaci vrstva například SAP ERP vývojového systému SAP BW testovací systém, produkční systému SAP CRM, atd... V Azure nasazení, které není podporováno toodivide tyto dvě vrstvy mezi místními a Azure. To znamená, že systému SAP buď je nasazena místně nebo je nasazené v Azure. Můžete však nasadit hello různých systémech šířku SAP do Azure nebo místní. Například může nasadit systémy vývoj a testování SAP CRM hello v Azure, ale hello SAP CRM produkční systému místní.
...
* Hello SAP systémy (a VM(s)) představují samostatné scénář v Azure, která pouze vyžaduje veřejné připojení k Internetu pro přístup k koncového uživatele a nevyžaduje tooother připojení virtuálních počítačů v Azure.
...
> * Některé součásti SAP vrstvy v Azure a některé na místě. Například rozdělení instance hello SAP aplikační vrstvu mezi místní a virtuální počítače Azure.
> * Není dostupná podpora distribuce virtuálních počítačů spuštěné instance SAP jeden systém nad několika oblastmi Azure.
>
> Hello důvod pro tato omezení je hello požadavek pro síť vysoký výkon, velmi nízkou latencí v rámci jednoho systému SAP, zejména mezi instancemi aplikace hello a hello databázového systému vrstvy systému SAP.
>
>
...
Hello platformy Azure omezuje hello nutnost počáteční technologie a nákupech infrastruktury. Zjednodušuje údržbu a provoz aplikace poskytnutím toohost výpočetního prostředí a úložiště na vyžádání, škálovat a spravovat webové aplikace a propojených aplikací. Správu infrastruktury je automatické s platformu, která je určená pro vysokou dostupnost a dynamické škálování potřeby využití toomatch s hello možnost průběžnými platbami cenový model.
...
Z provozního hlediska vyskytne hello, které služba Azure Virtual Machine poskytuje podobné jako virtuální počítače nasazené místně. Má však hello významné výhody, které nepotřebujete tooprocure, spravovat a správa infrastruktury hello. Vývojáři a správci mít plnou kontrolu nad hello bitovou kopii operačního systému v rámci těchto virtuálních počítačů. Správce může přihlásit vzdáleně do těchto virtuálních počítačů tooperform údržby a řešení potíží s úlohy a také úlohy nasazení softwaru. V ohledem toodeployment jsou hello pouze omezení velikosti hello a možnosti virtuálních počítačích Azure. Nemusí se jednat o jako jemné granulární v konfiguraci, jako tomu lze místně. Při volbě typů virtuálních počítačů, které představují kombinaci:
...
velikost Hello a omezení velikostí různé virtuální počítače různých nabízí, můžete zobrazit v tabulce v [v tomto článku][virtual-machines-sizes]

Jak budou si myslíte, že existují různé rodiny nebo řadu virtuálních počítačů. Od prosince 2015 lze rozlišit hello následující rodiny virtuálních počítačů:
...
* Počítač D-Series typy: lépe než A0 A7 provádění. Ne všechny typy hello virtuálních počítačů jsou certifikované s SAP.
* Typy virtuálních počítačů DS-Series: použijte stejný hostitele jako D-series, ale jsou možné tooconnect tooAzure Storage úrovně Premium (naleznete v kapitole [Azure Premium Storage] [ planning-guide-3.3.2] tohoto dokumentu). Znovu ne všechny typy virtuálních počítačů, které jsou certifikované s SAP.
...
Upozorňujeme, že všechny různé řady virtuálních počítačů může nabízena v každé z nich hello oblasti Azure (pro oblasti Azure naleznete v kapitole Další). Nezapomínejte, že ne všechny virtuální počítače nebo řadu virtuálních počítačů jsou certifikovány pro SAP.
...
Microsoft umožňuje toodeploy virtuálních počítačů do tak volané Azure oblasti. Oblast Azure může být jeden nebo více datových center, které se nacházejí v těsné blízkosti. Pro většinu hello geopolitické oblasti v hello, world Microsoft má alespoň dva oblasti Azure. Například v Evropě je oblast Azure, Severní Evropa, a jednu z "Západní Evropa". Tyto dvě Azure oblastem v geopolitické oblasti jsou odděleny dostatečně velkou vzdálenost tak, aby přírodních nebo technických havárie neovlivní obou oblastí Azure v hello stejné geopolitické oblasti. Vzhledem k tomu, že Microsoft vytrvale vytváří se nové oblasti Azure v různých oblastech geopolitické globálně, hello počet tyto oblasti vytrvale roste a od prosince 2015 dosaženo hello počtu 20 oblasti Azure s další oblasti oznámeno již. Systémy SAP vy jako zákazník můžete nasadit do všech těchto oblastech, včetně hello dvou oblastech Azure v Číně. Pro aktuální až toodate informace o oblastech Azure naleznete v tomto webu: <https://azure.microsoft.com/regions/>

### <a name="8d8ad4b8-6093-4b91-ac36-ea56d80dbf77"></a>Hello koncept virtuálního počítače Microsoft Azure
Microsoft Azure nabízí infrastruktury jako toohost řešení služby (IaaS) virtuálních počítačů s podobné funkce jako místní řešení virtualizace. Se může toocreate virtuální počítače v rámci hello portálu Azure, PowerShell nebo rozhraní příkazového řádku, které také nabízí nasazení a možnosti správy.
...
* [Nasazení a správě virtuálních počítačů pomocí šablon Azure Resource Manageru a hello rozhraní příkazového řádku Azure][virtual-machines-linux-cli-deploy-templates]
...
Microsoft Azure virtuální počítače využívat různé typy úložišť. Při implementaci SAP na služby virtuální počítač Azure je důležité toounderstand hello rozdíly mezi tyto dva hlavní typy úložiště:
...
Je možné tooupload existujícího virtuálního pevného disku z místní nebo vytvořit prázdných z v rámci Azure a připojte tyto virtuální počítače toodeployed. Tyto virtuální pevné disky jsou odkazovány jako disky Azure.
...
* Hello zóny redundantní úložiště, který se šíří hello tři Image přes různých datových střediscích v rámci stejné oblasti Azure.
* Výchozí úroveň redundance je geografická redundance, která asynchronně replikuje hello obsahu do jiného 3 bitové kopie hello data do jiné oblasti Azure, který je hostován v hello stejné geopolitické oblasti.

Také zjistit hello tabulky v tomto článku v možnostech různých redundance toohello namapoval: <https://azure.microsoft.com/pricing/details/storage/>

Další informace v tooAzure namapoval, které úložiště najdete tady:
...
Úložiště Azure BLOB standardní byl hello typu úložiště k dispozici, pokud byl vydán Azure IaaS. Nebyly IOPS kvóty za jeden virtuální pevný disk. Latence došlo nebyla ve hello stejné třídy jako zařízení sítě SAN nebo NAS obvykle nasazují systémů SAP vyšší kategorie hostované na místě. Nicméně hello Azure Standard Storage prokázat dostatečná pro mnoho stovky systémy SAP mezitím nasazené v Azure.

Azure Standard Storage je účtovat podle hello skutečná data, která je uložena, objem hello transakce úložiště, odchozí přenosy dat a zvolená možnost redundance. Lze vytvořit mnoho virtuálních pevných disků v hello maximálně 1 TB, velikost, ale také ty zůstat prázdné je bezplatná. Pokud pak zadejte jeden virtuální pevný disk s 100GB, pro ukládání 100GB a ne pro hello hello nominální velikost virtuálního pevného disku nebyl vytvořen s vám bude účtována.
...
K tomuto účelu byly zavedeny velké množství změn, které hello dva nejvýznamnějších jsou:
...
V opačné tooStandard úložiště, kde se nezměnila možnosti závisí na velikosti hello hello disku (nebo VHD), Storage úrovně Premium aktuálně má 3 jiný disk kategorií, které se zobrazují na konci hello tohoto článku před části hello – nejčastější dotazy: <https:/ /Azure.microsoft.com/pricing/details/Storage/>

Uvidíte, zda jsou závislé na kategorie hello velikost disků hello IOPS/virtuálního pevného disku a disku propustnost nebo virtuální pevný disk

Základní náklady v případě hello úložiště Premium Storage není svazek hello skutečná data uložená v takových virtuálních pevných disků, ale velikost kategorii hello takové virtuální pevný disk, nezávisle na hello množství hello data, která je uložena v hello virtuálního pevného disku.

Také můžete vytvořit virtuální pevné disky na Storage úrovně Premium, které nejsou přímo mapování do kategorií velikost hello vidět. To může být případ hello, zejména v případě, že kopírování virtuálních pevných disků ze standardního úložiště do úložiště úrovně Premium. V takových případech se provádí disku řešením mapování toohello další největší úložiště Premium.

Upozorňujeme, že pouze určité řadu virtuálních počítačů mohou těžit z výhod hello Azure Premium Storage. Od prosince 2015 jedná se o hello DS - a GS-series. Hello DS-series je v podstatě hello stejné jako D-series s výjimkou hello že DS-series má možnost toomount hello Storage úrovně Premium založené na virtuálních počítačích kromě tooVHDs, které jsou hostované v Azure Standard Storage. Samé platí pro porovnání G-series tooGS-series.

Pokud se odhlašuje hello součástí hello DS-series virtuálních počítačů v [v tomto článku] [ virtual-machines-sizes] byste se si uvědomit, že neexistují data svazku omezení tooPremium úložiště virtuálních pevných disků na hello členitost hello úroveň virtuálního počítače. Různé DS-series nebo GS-series virtuálních počítačů také mít různá omezení v namapoval toohello počet virtuálních pevných disků, které může být připojen. Tato omezení jsou popsané v článku hello zmíněné také. Ale v podstatě znamená, že pokud se například připojit 32 virtuálních pevných disků nebo disků tooa x P30 jeden virtuální počítač DS14 není získáte 32 x hello maximální propustnost P30 disku. Místo toho hello maximální propustnost na úrovni virtuálních počítačů, jak je uvedeno v článku hello omezí propustnost dat.
...
Microsoft Azure se zajišťují infrastrukturu sítě, která umožní mapování hello všechny scénáře, které chceme toorealize softwarem SAP. Hello možnosti jsou:
...
Existuje mnoho různých možnosti tooconfigure názvu a IP řešení v Azure. V tomto dokumentu jenom pro Cloud scénáře spoléhají na výchozí hello pomocí Azure DNS (v kontrastu toodefining služby vlastní DNS). Je také novou službu Azure DNS, který může být použit místo nastavení serveru DNS. Další informace naleznete v [v tomto článku] [ virtual-networks-manage-dns-in-vnet] a na [tuto stránku](https://azure.microsoft.com/services/dns/).

Pro scénáře mezi různými místy, které jsme se spoléhat na hello fakt, které hello místní AD a OpenLDAP/DNS rozšířilo prostřednictvím sítě VPN nebo tooAzure privátní připojení. Pro určité scénáře podle postupu uvedeného v tomto poli může být nutné toohave repliku AD nebo OpenLDAP nainstalován v Azure.
...
Sestavením virtuální síť Azure můžete definovat rozsah adres hello hello soukromé IP adresy přidělené funkce Azure protokolu DHCP. Ve scénářích mezi různými místy rozsah IP adres hello definované stále se přidělí pomocí protokolu DHCP v Azure. Překlad názvu domény však bude provedeno místní (za předpokladu, že hello virtuální počítače jsou součástí místní domény) a proto může překládat adresy mimo jiné cloudové služby Azure.
...
V pořadí toocreate připojení site-to-site (místní data center tooAzure datového centra), budete potřebovat tooeither získat a nakonfigurovat zařízení VPN nebo použít směrování a vzdálený přístup (RRAS) zavedenou jako součást softwaru se systémem Windows Server 2012.
...
Point-to-site VPN vyžaduje každý počítač tooconnect klienta s vlastním VPN do Azure. Pro scénáře SAP hello Těšíme se na není praktické připojení point-to-site. Žádné další odkazy. proto bude mít toopoint-to-site VPN připojení.
...
Budeme potřebovat toobe vymazat o hello fakt této hello úložiště a síťové infrastruktury jsou sdílena mezi virtuální počítače se systémem řady služeb v hello infrastruktury Azure. A jako v případě hello zákazníka vlastní datových centrech předimenzování některých prostředků infrastruktury hello trvá stupeň tooa místní. Hello platforma Microsoft Azure používá disk, procesoru, sítě a jiných spotřeby prostředků hello toolimit kvóty a výkonu toopreserve konzistentní a deterministický.  Hello různé virtuálních počítačů typy (A5, A6 atd.) mají různé kvóty pro hello počet disků, procesoru, paměti RAM a sítě.

> [!NOTE]
> Prostředky procesoru a paměti hello typů virtuálních počítačů nepodporuje SAP jsou předběžně přidělených na uzly hostitele hello. To znamená, že po nasazení virtuálního počítače hello hello prostředky na hostiteli hello bude k dispozici podle definice hello typ virtuálního počítače.
>
>

Je třeba zvážit při plánování a změna velikosti SAP na řešení Azure hello kvóty pro každou velikost virtuálního počítače.  kvóty Hello virtuálních počítačů jsou popsány [sem][virtual-machines-sizes].

Hello kvót popsané představují hello teoretický maximální hodnoty.  Hello limit IOPS na virtuální pevný disk lze dosáhnout s malé IOs (8kb), ale pravděpodobně nemusí být dosaženo s velké IOs (1 Mb).  Hello IOPS limit se vynucuje u hello členitost jedné virtuální pevné disky.
...
Pro nově nasazené systémy SAP by měl prošli velikosti cvičení, která by měla určí požadavky protokoly SAP hello hello systému.
...
**Krok 4**: jak se píše [sem][virtual-machines-sizes], Azure vynucuje kvóty IOPS podle nezávislých na virtuální pevný disk, zda používat úložiště úrovně Standard nebo Premium Storage. Závisí na hello typ virtuálního počítače, hello počet virtuálních pevných disků, které může být připojen se liší. V důsledku toho můžete vypočítat maximální IOPS číslo, které lze dosáhnout s jednotlivými hello různé typy virtuálních počítačů. Závisí na hello rozložení souboru databáze, vám může rozkládají virtuální pevné disky toobecome jeden svazek v hello hostovaný operační systém. Ale pokud hello aktuální IOPS svazku nasazené systému SAP překračuje omezení hello vypočítat hello největší typ virtuálního počítače Azure a pokud neexistuje žádné prvního toocompensate s více paměti, hello zatížení hello systému SAP ovlivněné můžou být vážně. V takových případech můžete dosáhl bodu, kde není vhodné nasazovat hello systému v Azure.

**Krok 5**: hlavně v SAP systémy, které jsou nasazené na místě v konfiguracích vrstvě 2, hello pravděpodobné že hello systému může být nutné toobe nakonfigurovaný v Azure v konfiguraci vrstvy 3. V tomto kroku je nutné toocheck, zda je součást v hello SAP aplikační vrstvu, která nelze škálovat na více systémů a který by začlenit do hello procesoru a paměti, že prostředky hello různé typy nabídka virtuálního počítače Azure. Pokud skutečně existuje takový komponentu, hello SAP systému a jeho úlohy nelze nasadit do Azure. Pokud jste můžete Škálováním na více systémů, ale hello SAP aplikaci, kterou součásti do více virtuálních počítačů Azure, hello systém lze nasadit do Azure.
...
Přečtěte si také kapitoly [rozhraní příkazového řádku Azure pro virtuální počítače s Linuxem] [ deployment-guide-4.5.2] v hello [Průvodce nasazením] [ planning-guide] na tom, jak toouse rozhraní příkazového řádku Azure toodeploy hello Azure Rozšíření monitorování pro SAP.

## <a name="different-ways-toodeploy-vms-for-sap-in-azure"></a>Různé způsoby toodeploy virtuálních počítačů pro SAP v Azure
V této kapitole se dozvíte hello různé způsoby toodeploy virtuálního počítače v Azure. V této kapitole jsou popsané postupy další přípravy, jakož i zpracování virtuální pevné disky a virtuální počítače v Azure.

### <a name="deployment-of-vms-for-sap"></a>Nasazení virtuálních počítačů pro SAP
Microsoft Azure nabízí několik způsobů toodeploy virtuální počítače a přidružené disky. Proto je velmi důležité toounderstand hello rozdíly, od přípravy hello virtuálních počítačů se můžou lišit v závislosti na metodě hello nasazení. Obecně platí jsme se podívejte se na hello následující scénáře:

#### <a name="4d175f1b-7353-4137-9d2f-817683c26e53"></a>Přesunutí virtuálního počítače z místní tooAzure s diskem zobecněn
Máte v plánu toomove konkrétního systému SAP z místní tooAzure. To lze provést tím, že nahrajete hello virtuálního pevného disku, který obsahuje hello operačního systému, hello SAP binární soubory a databázového systému binární soubory plus hello virtuální pevné disky s hello data a soubory hello databázového systému tooAzure protokolu. Na rozdíl od příliš[scénář #2 níže][planning-guide-5.1.2], zachovat hello název hostitele, identifikátor SID SAP a SAP uživatelské účty v hello virtuálního počítače Azure, jak byla nakonfigurována v místním prostředí hello. Proto generalizací hello bitové kopie není nutné. Najdete v tématu kapitolám [přípravy pro přesun virtuálního počítače z místní tooAzure s diskem zobecněn] [ planning-guide-5.2.1] tohoto dokumentu pro místní přípravné kroky a odesílání-zobecněný virtuální počítače nebo virtuální pevné disky tooAzure. Přečtěte si kapitolu [scénář 3: přesunutí virtuálního počítače z místního virtuálního pevného disku není zobecněný Azure pomocí SAP] [ deployment-guide-3.4] v hello [Průvodce nasazením] [ deployment-guide]podrobný postup nasazení takové bitovou kopii v Azure.
...
* Virtuální pevné disky, které jsou připojené toohello virtuálních počítačů a by měl být připojené znovu toobe nutné virtuální počítač Azure toohello v také pevném formátu virtuálního pevného disku. Hello stejné omezení velikosti disku operačního systému hello platí i toodata disky. Virtuální pevné disky může mít maximální velikost 1TB. Dynamické virtuální pevné disky bude převedený toostatic virtuálních pevných disků při nahrávání hello virtuální pevný disk pomocí rutiny prostředí PowerShell nebo rozhraní příkazového řádku
...
* Pro případ hello použití scénáři jenom pro Cloud nasazení (naleznete v kapitole [jenom pro Cloud - nasazení virtuálních počítačů do Azure bez závislosti na hello místní síti zákazníka] [ planning-guide-2.1] tohoto zdokumentujte) v kombinaci s Tato metoda nasazení, domény, které účty nemusí fungovat po hello disku Azure je nasazené v Azure. To platí hlavně pro účty, které jsou používané toorun služby, jako je aplikace hello databázového systému nebo SAP. Proto nutné tooreplace tyto účty domény s účty místní počítač a odstraňte hello místní doménové účty v hello virtuálních počítačů. Zachování místní domény uživatele v hello image virtuálního počítače se nejedná o problém při hello virtuálních počítačů je nasazení do scénáře hello mezi různými místy, jak je popsáno v kapitole [mezi různými místy - nasazení jedné nebo více virtuálních počítačů SAP do Azure s hello požadavek se plně integrován do místní sítě hello] [ planning-guide-2.2] v tomto dokumentu.
...
> Ujistěte se, zda se nepoužívá/mnt nebo prostředků a že jsou všechny disky připojené přes uuid. Pro disk hello operačního systému ověřte, že položka hello zaváděcí program pro spouštění také změna připojení na základě uuid hello.
>
>
...
Soubory virtuálního pevného disku, které obsahují zobecněný operačního systému jsou uloženy i v kontejnery v účtech úložiště Azure. Můžete nasadit nový virtuální počítač z takových image virtuálního pevného disku jako zdroj virtuálního pevného disku v souborech nasazení šablony odkazující na hello virtuálního pevného disku, jak je popsáno v kapitole [scénář 2: nasazení virtuálního počítače s vlastní image pro SAP] [ deployment-guide-3.3] z hello [Průvodce nasazením][deployment-guide].
...
> ![Linux][Logo_Linux] Linux
>
> Ujistěte se, zda se nepoužívá/mnt nebo prostředků a že jsou všechny disky připojené přes uuid. Hello operačního systému disku ujistěte, že položka zaváděcí program pro spouštění hello také odráží připojení na základě uuid hello.
>
>
...
* Další software, nezbytné toorun hello virtuální počítače úspěšně ve scénářích mezi různými místy lze nainstalovat, dokud tento software může spolupracovat s hello přejmenujte z hello virtuálních počítačů.
...
> ![Windows][Logo_Windows] Windows
>
> posledním krokem Hello je toolog v tooa virtuálních počítačů s účtem správce. Otevřete příkazové okno Windows jako 'správce. Přejděte too...\windows\system32\sysprep a provést sysprep.exe.
...
> Pokud chcete tooperform hello postup v případě virtuálních počítačů už běží v Azure, postupujte podle kroků hello popsaných v [v tomto článku][virtual-machines-windows-capture-image].
>
> ![Linux][Logo_Linux] Linux
>
> [Jak toocapture toouse virtuálního počítače Linux jako šablony Resource Manageru][virtual-machines-linux-capture-image]
>
>
...
* Předplatné tooyour přihlášení s *Login-AzureRmAccount*
* Nastavte předplatné hello váš kontext s *Set-AzureRmContext* a parametr ID předplatného nebo Název_předplatného - <https://msdn.microsoft.com/library/mt619263.aspx>
* Nahrát hello virtuálního pevného disku s *přidat AzureRmVhd* najdete v části tooan účet úložiště Azure - <https://msdn.microsoft.com/library/mt603554.aspx>
* Sada hello operačního systému disku toohello nové konfigurace virtuálních počítačů virtuálního pevného disku s *Set-AzureRmVMOSDisk* -najdete v části <https://msdn.microsoft.com/library/mt603746.aspx>
* Vytvoření nového virtuálního počítače z konfigurace virtuálního počítače hello s *New-AzureRmVM* -najdete v části <https://msdn.microsoft.com/library/mt603754.aspx>
* Přidat datového disku tooa nový virtuální počítač s *přidat AzureRmVMDataDisk* -najdete v části <https://msdn.microsoft.com/library/mt603673.aspx>

**Azure CLI**

* Přepínač režimu Resource Manager tooAzure s *azure config režimu arm*
* Předplatné tooyour přihlášení s *přihlášení k azure*
* Vyberte předplatné s *nastaven účet azure`<subscription name or id`>*
* Nahrát hello virtuálního pevného disku s *nahrání objektu blob úložiště azure* -najdete v části [hello pomocí rozhraní příkazového řádku Azure s Azure Storage][storage-azure-cli]
* Vytvoření nového virtuálního počítače zadání hello odesílané virtuální pevný disk jako disk operačního systému s *vytvoření virtuálního počítače azure* a parametr -d
* Přidat datového disku tooa nový virtuální počítač s *virtuálních počítačů disku připojení nového*

**Šablona**

* Nahrát hello virtuálního pevného disku pomocí prostředí Powershell nebo rozhraní příkazového řádku Azure
* Nasazení hello virtuálních počítačů pomocí JSON šablony odkazující na hello virtuálního pevného disku, jak je znázorněno v [této šablony JSON příklad](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/201-vm-from-specialized-vhd/azuredeploy.json).

#### <a name="deployment-of-a-vm-image"></a>Nasazení Image virtuálního počítače
tooupload existující virtuální počítač nebo virtuální pevný disk z hello místní sítě v pořadí toouse ji jako bitovou kopii virtuálního počítače Azure virtuálních počítačů nebo virtuální pevný disk potřebujete toomeet hello požadavky uvedené v kapitole [přípravy pro nasazení virtuálního počítače s konkrétní image zákazníka pro SAP] [ planning-guide-5.2.2] tohoto dokumentu.

* Použití *sysprep* v systému Windows nebo *příkaz waagent-deprovision* na Linux toogeneralize virtuálního počítače – najdete v části [jak toocapture virtuálního počítače s Windows v hello nasazení Resource Manager modelu] [ virtual-machines-windows-capture-image-prepare-the-vm-for-image-capture] nebo [jak toocapture toouse virtuálního počítače Linux jako šablony Resource Manageru][virtual-machines-linux-capture-image-capture]
* Předplatné tooyour přihlášení s *Login-AzureRmAccount*
* Nastavte předplatné hello váš kontext s *Set-AzureRmContext* a parametr ID předplatného nebo Název_předplatného - <https://msdn.microsoft.com/library/mt619263.aspx>
* Nahrát hello virtuálního pevného disku s *přidat AzureRmVhd* najdete v části tooan účet úložiště Azure - <https://msdn.microsoft.com/library/mt603554.aspx>
* Sada hello operačního systému disku toohello nové konfigurace virtuálních počítačů virtuálního pevného disku s *Set AzureRmVMOSDisk - SourceImageUri - CreateOption fromImage* -najdete v části <https://msdn.microsoft.com/library/mt603746.aspx>
* Vytvoření nového virtuálního počítače z konfigurace virtuálního počítače hello s *New-AzureRmVM* -najdete v části <https://msdn.microsoft.com/library/mt603754.aspx>

**Azure CLI**

* Použití *sysprep* v systému Windows nebo *příkaz waagent-deprovision* na Linux toogeneralize virtuálního počítače – najdete v části [jak toocapture virtuálního počítače s Windows v hello nasazení Resource Manager modelu] [ virtual-machines-windows-capture-image-prepare-the-vm-for-image-capture] nebo [jak toocapture toouse virtuálního počítače Linux jako šablony Resource Manageru] [ virtual-machines-linux-capture-image-capture] pro Linux
* Přepínač režimu Resource Manager tooAzure s *azure config režimu arm*
* Předplatné tooyour přihlášení s *přihlášení k azure*
* Vyberte předplatné s *nastaven účet azure`<subscription name or id`>*
* Nahrát hello virtuálního pevného disku s *nahrání objektu blob úložiště azure* -najdete v části [hello pomocí rozhraní příkazového řádku Azure s Azure Storage][storage-azure-cli]
* Vytvoření nového virtuálního počítače zadání hello odesílané virtuální pevný disk jako disk operačního systému s *vytvoření virtuálního počítače azure* a parametru -Q

**Šablona**

* Použití *sysprep* v systému Windows nebo *příkaz waagent-deprovision* na Linux toogeneralize virtuálního počítače – najdete v části [jak toocapture virtuálního počítače s Windows v hello nasazení Resource Manager modelu] [ virtual-machines-windows-capture-image-prepare-the-vm-for-image-capture] nebo [jak toocapture toouse virtuálního počítače Linux jako šablony Resource Manageru] [ virtual-machines-linux-capture-image-capture] pro Linux
* Nahrát hello virtuálního pevného disku pomocí prostředí Powershell nebo rozhraní příkazového řádku Azure
* Nasazení hello virtuálních počítačů pomocí JSON šablony odkazující na hello image virtuálního pevného disku, jak je znázorněno v [této šablony JSON příklad](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/101-vm-from-user-image/azuredeploy.json).

#### <a name="downloading-vhds-tooon-premises"></a>Stahování tooon místní virtuální pevné disky
...
Během doby hello hello stažení hello virtuální pevné disky nesmí být aktivní. I když virtuální pevné disky, které jsou připojené tooVMs stahuje hello virtuálních počítačů musí toobe vypnutí. Pokud chcete pouze obsah, pak by se použít tooset do nového systému místní a pokud je možné, že během hello čas hello stáhnout a hello instalace hello nový systém, který hello systému v Azure může být stále provozní databáze hello toodownload , můžete dlouho výpadky provedením zálohy komprimované databáze do virtuální pevný disk a stáhnout pouze, že virtuální pevný disk místo také stahování hello základní virtuální počítač operačního systému.

#### <a name="powershell"></a>PowerShell
Po zastavení systému SAP hello a hello virtuálního počítače je vypnutí, můžete použít rutiny prostředí PowerShell hello AzureRmVhd uložit na hello místního disků VHD hello toodownload target zpět toohello místní world. V pořadí toodo, že hello URL hello virtuálního pevného disku, které můžete najít v hello 'části úložiště, musíte z hello portálu Azure (třeba toonavigate toohello účet úložiště a hello kontejner úložiště kde hello VHD se vytvořil) a je nutné tooknow kde hello virtuálního pevného disku by měla být zkopírovat do.
...
Další podrobnosti hello uložit AzureRmVhd rutiny, zkontrolujte, zde <https://msdn.microsoft.com/library/mt622705.aspx>.

#### <a name="cli"></a>Rozhraní příkazového řádku
Po zastavení systému SAP hello a hello virtuálního počítače je vypnutí, můžete použít ke stažení hello rozhraní příkazového řádku Azure příkaz úložiště azure blob na hello místního disků VHD hello toodownload target zpět toohello místní world. V pořadí toodo, že potřebujete z portálu Azure (třeba toonavigate toohello účet úložiště a hello kontejner úložiště kde hello VHD se vytvořil) hello hello název a hello kontejner hello virtuálního pevného disku, které můžete najít v hello 'části úložiště, a je nutné tooknow kde Hello virtuálního pevného disku by se měl zkopírovat do.
...
Systému SAP nebo i vyhrazený server databázového systému podpora aplikační vrstvu SAP pravděpodobně obsahovat několik virtuálních pevných disků, které obsahují buď hello operačního systému s binárními soubory hello nebo hello data a soubory databáze SAP hello protokolu. Hello Azure funkce kopírování virtuálních pevných disků ani hello Azure funkce ukládání virtuálních pevných disků toodisk má synchronizační mechanismus, který by snímku více virtuálních pevných disků synchronně. Proto stav hello hello zkopírovat nebo uloženy virtuální pevné disky, i v případě, že těch, které jsou připojené proti hello stejného virtuálního počítače by být odlišné. To znamená, že v případě konkrétní hello toho, že různé datové a logfile(s) obsažené v hello jiný virtuální pevné disky, hello databáze v hello end by být nekonzistentní.

**Uzavření: V pořadí toocopy nebo uložte virtuální pevné disky, které jsou součástí konfigurace aplikace SAP systému musíte toostop hello SAP systému a také musí nasadit tooshut dolů hello virtuálních počítačů. Pouze pak můžete zkopírovat nebo stáhnout hello sadu tooeither virtuální pevné disky vytvoření kopie systému SAP hello v Azure nebo na místní.**

Datové disky jsou uloženy jako soubory virtuálního pevného disku v účtu úložiště Azure a může být přímo připojit tooa virtuálního počítače nebo použít jako obrázek. V takovém případě hello virtuálního pevného disku je umístění zkopírovaného tooanother před beeing připojené toohello virtuálního počítače. Hello úplný název souboru virtuálního pevného disku hello v Azure musí být jedinečný v rámci Azure. Jak už bylo zmíněno dříve již, název hello je druh třemi částmi název, který vypadá takto:

    http(s)://<storage account name>.blob.core.windows.net/<container name>/<vhd name>

##### <a name="powershell"></a>PowerShell
Můžete použít toocopy rutin prostředí Azure PowerShell virtuální pevný disk, jak je znázorněno v [v tomto článku][storage-powershell-guide-full-copy-vhd].

##### <a name="cli"></a>Rozhraní příkazového řádku
Můžete použít rozhraní příkazového řádku Azure toocopy virtuální pevný disk, jak je znázorněno v [v tomto článku][storage-azure-cli-copy-blobs]
...
Tuto úlohu nelze provést na hello portálu Azure. Můžete ise prostředí Azure PowerShell rutin Azure CLI nebo třetí strany úložiště prohlížeče. Hello rutiny prostředí PowerShell nebo rozhraní příkazového řádku můžete vytvářet a spravovat objekty BLOB, mezi které patří objekty BLOB hello možnost tooasynchronously kopírování mezi různými účty úložiště a v oblastech v rámci hello předplatného Azure.
...
* Vytvoření kontextu účtu úložiště pro účet úložiště zdroj hello *New-AzureStorageContext* -najdete v části <https://msdn.microsoft.com/library/dn806380.aspx>
* Vytvoření kontextu účtu úložiště pro hello cílový účet úložiště s *New-AzureStorageContext* -najdete v části <https://msdn.microsoft.com/library/dn806380.aspx>
...
V ideálním případě hello zpracování struktury hello virtuálního počítače a hello přidružené virtuální pevné disky musí být velmi jednoduché. V místní instalace vyvinuté zákazníkům mnoho způsobů strukturování instalaci serveru.

* Jeden základní virtuální pevný disk obsahující hello operačního systému a všechny binární soubory hello z hello databázového systému nebo SAP. Tento virtuální pevný disk lze až too1TB velikost místo starší omezení, která je omezená too127GB od března 2015.
...
* Počet virtuálních pevných disků (Pokud hello databázového systému podporuje to) obsahující jeden nebo dva soubory databáze z databáze SAP hello a hello databázového systému dočasná data i soubory.
...
- - -
Hello počet virtuálních pevných disků použitých pro hello databázového systému datové soubory a typ hello tyto virtuální pevné disky hostované v Azure Storage měli určit hello IOPS požadavků a hello latence vyžaduje. Přesný kvóty jsou popsané v [v tomto článku][virtual-machines-sizes]

Prostředí SAP nasazení přes hello poslední 2 roky nám výukové některé lekce, které jde vyhodnotit jako:

* IOPS provoz toodifferent datových souborů není vždy hello stejné od stávajících systémů zákazníků může mít jinak velikost dat souborů představující jejich SAP databází. V důsledku zapnuta out toobe více virtuálních pevných disků tooplace hello datové soubory, které logické jednotky LUN carved mimo těch, které lépe pomocí konfiguraci RAID. Nebyly situacích, zejména s Azure Standard Storage, kde IOPS míra dosáhl kvóty hello jednoho virtuálního pevného disku proti hello databázového systému transakčního protokolu. Tyto scénáře použití hello úložiště Premium se doporučuje nebo případně agregování více standardní úložiště virtuálních pevných disků s softwaru diskového pole RAID.
...
Mějte na paměti této hello virtuálního pevného disku, který obsahuje hello operačního systému, a jako doporučujeme, hello binární soubory databáze SAP a hello (základní virtuální počítač) taky není už omezený too127GB. Teď může mít až too1TB velikost. Měl by být tookeep dostatek místa na všech hello nezbytné souboru, včetně například SAP batch v protokolech úloh.
...
#### <a name="disk-handling"></a>Zpracování disku
Ve většině případů potřebujete další disky toocreate v databázi SAP hello toodeploy pořadí do hello virtuálních počítačů. Už jsme mluvili o aspektech hello na počet virtuálních pevných disků v kapitole [struktura virtuálního počítače nebo virtuálního pevného disku pro nasazení SAP] [ planning-guide-5.5.1] tohoto dokumentu. Hello portálu Azure umožňuje tooattach a odpojit disky po základní virtuální počítač nasazen. Hello disků může být připojit nebo odpojit po hello virtuálních počítačů a spuštěných i když je zastavena. Při připojení disku, hello portál Azure poskytuje tooattach prázdný disk nebo stávající disk, který v tuto chvíli není připojen tooanother virtuálních počítačů.

**Poznámka:**: virtuálních pevných disků může být pouze připojené tooone virtuálních počítačů v každém okamžiku.

![Připojit / Odpojit disky s Azure Standard Storage][planning-guide-figure-1400]

Jestli chcete toocreate nové a prázdný virtuální pevný disk (které by být vytvořeny v hello stejný účet úložiště jako hello základní virtuální počítač je v) nebo jestli chcete, aby byl tooselect existující virtuální pevný disk, který se předtím nahrála a měl by být připojen toohello virtuálního počítače teď musíte toodecide.
...
> ![Windows][Logo_Windows] Windows
>
> [Jak tooattach dat na disku v hello portálu Azure][virtual-machines-windows-attach-disk-portal]
>
> Pokud jsou připojené disky, musíte toolog v do hello virtuálních počítačů tooopen hello Správce disků systému Windows. Pokud automatického připojení není povolena podle doporučení v kapitole [nastavení automatického připojení pro připojené disky][planning-guide-5.5.3], hello nově připojená svazku musí toobe provést online a inicializován.
>
> ![Linux][Logo_Linux] Linux
>
> Pokud jsou připojené disky, potřebujete toolog v do hello virtuálního počítače a inicializace hello disků, jak je popsáno v [v tomto článku][virtual-machines-linux-how-to-attach-disk-how-to-initialize-a-new-data-disk-in-linux]
>
>
...
>
> okno příkazového řádku Windows Hello musí být otevřen jako správce.
>
...
> * V posledním kroku hello hello průvodce, je nutné toogive hello název pravidla a potom uložte pravidlo hello stisknutím kombinace kláves 'Dokončit'
>
...
#### <a name="setup-network-for-communication-between-hello-different-vms"></a>Nastavení sítě pro komunikaci mezi hello různé virtuální počítače
...
scénář mezi různými místy Hello lze zhruba popsat jako hello grafického níže:

![Připojení Site-to-Site mezi místními a prostředky Azure][planning-guide-figure-2100]

Hello scénář uvedené výše popisuje scénář, kde hello místní AD nebo OpenLDAP a DNS je rozšířeno tooAzure. Na straně místní hello je vyhrazený určitý rozsah IP adres podle předplatného Azure. Hello rozsah IP adres se přiřadí tooan Azure Virtual Network na hello Azure straně.
...
> * postupujte podle hello veřejné Linux příručky pro [SUSE](https://www.suse.com/documentation/sles-12/book_sle_deployment/data/sec_y2_hw_print.html) nebo [Red Hat](https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/6/html/Deployment_Guide/sec-Printer_Configuration.html) o tooadd tiskárnu.
>
>
...
Mezi různými místy způsoby připojené site-to-site hello latenci mezi místními a Azure stále může být výrazně. Pokud jsme postupujte podle hello posloupnost převod objekty prostřednictvím vývoj a testování tooproduction systémy nebo myslíte o použití přenosy nebo podporu balíčky toohello různými systémy, si myslíte, závisí na umístění hello hello centrální přenosu adresář, některé systémy hello se setkají vysokou latencí čtení nebo zápis dat v adresáři centrální přenosu hello. Hello situace je podobné tooSAP na šířku konfigurace, kde se šíří různými systémy hello prostřednictvím různých datových střediscích s dlouhou vzdálenost mezi datovými centry hello.
...
RFC provoz mezi systémy, které jsou místně a v Azure potřebuje toowork. toosetup připojení volání transakce SM59 ve zdrojovém systému kde je nutné toodefine připojení k RFC směrem hello cílového systému. Konfigurace Hello je podobné toohello standardní nastavení připojení k RFC.
...
Od února 2014 hello Agent virtuálního počítače Azure umožňující zpracování konkrétní rozšíření virtuálního počítače Azure v rámci hello virtuálního počítače je vloženy do virtuálních počítačů Windows ve výchozím nastavení při vytváření virtuálních počítačů v hello portálu Azure. V případě SUSE nebo Red Hat Linux hello agenta virtuálního počítače je již součástí bitové kopie Azure Marketplace. V případě, že jeden by nahrát virtuální počítač s Linuxem z agenta virtuálního počítače hello tooAzure místní má toobe nainstalován ručně.
...
Počáteční portál Hello identifikátor URI je http (s):`<Portalserver`>: 5XX00/irj, kde je hello port tvořen 50000 plus (Systemnumber x 100). Hello výchozí portálu URI SAP systém 00 je `<dns name`>.`<azure region` >.Cloudapp.azure.com:PublicPort/irj. Další informace, podívejte se <http://help.sap.com/saphelp_nw70ehp1/helpdata/de/a2/f9d7fed2adc340ab462ae159d19509/frameset.htm>.
...
Není žádný SLA k jedním Virtuálním počítačům k dispozici ve virtuálních počítačích Azure nyní. tooget představu, jak hello dostupnost jeden virtuální počítač může vypadat jako můžete jednoduše vytvořit hello produkt hello různé dostupné Azure SLA: <https://azure.microsoft.com/support/legal/sla/>.
...
Vzhledem k tomu, že Azure Storage je ve výchozím nastavení uchovávání 3 bitové kopie dat hello, RAID5 nebo RAID1 na více disků Azure nejsou potřebné.
...
Každý SAP jako instance je umístěn v svůj vlastní účet služby Azure Storage – potenciální nedostupnosti jeden účet úložiště Azure způsobí, že nejsou dostupné jenom jeden virtuální počítač s jeho SAP AS instance. Však Upozorňujeme, že je maximální počet účtů úložiště Azure v rámci jednoho předplatného Azure. Automatické spuštění tooensure (A) instance SCS po restartování počítače hello virtuálních počítačů, ujistěte se, profil popsané v kapitole spuštění tooset parametr automatické spuštění v instanci (A) SCS [pomocí automatické spuštění pro instance SAP][planning-guide-11.5].
...
tooachieve úplné SAP systému vysokou dostupnost, potřebujeme tooprotect všechny kritické SAP součásti systému, například redundantní SAP aplikační servery, a jako jedinečný součásti (například jediné místo poruchy) instance SCS SAP (A) a databázového systému.
...
Neexistuje žádné nekonečné počet selhání a upgradu domén, které je možné podle skupiny dostupnosti Azure, v rámci jednotky škálování služby Azure. To znamená, že uvedení počet virtuálních počítačů do jedné sady dostupnosti dřív nebo později v hello skutečnost, že více než jeden virtuální počítač bude mít v hello stejné selhání nebo upgradu domény

Nasazení několika aplikačního serveru SAP instancí v jejich vyhrazených virtuálních počítačích a za předpokladu, že My 5 upgradu domény, na konci hello ukáže hello následující obrázek. Hello skutečný maximální počet domén selhání a aktualizace v rámci skupiny dostupnosti mohou změnit v budoucnu hello:
...
* Všechny virtuální počítače spuštěné instance (A) SCS jednoho systému SAP se nacházejí v jedné skupině dostupnosti. Uvnitř těchto virtuálních počítačů je nakonfigurovat instanci SCS tooprotect (A) systému Windows Server Failover Cluster (WSFC).
...
Hello architektura pro SAP HA v systému Linux v Azure je v podstatě stejný jako u systému Windows jako výše popsané hello. Od ledna 2016 existují ale dvě omezení:
...
Závisí na hello konfigurace SAP existuje vybrali (vrstvy 2 nebo 3 úrovně) může být nutné toobackup. obsah Hello hello virtuální počítač a toohave zálohu databáze hello. zálohy jsou očekávané toobe provádět pomocí metod databáze související s Hello databázového systému. Podrobný popis pro hello různých databází, najdete v [databázového systému průvodce][dbms-guide]. Na dobrý den druhé straně, hello SAP data lze zálohovat offline způsobem (včetně hello databáze také obsah) jak je popsáno v této části nebo online jak je popsáno v další části hello.

zálohování offline Hello by vyžadovaly v podstatě vypnutí hello virtuálních počítačů prostřednictvím hello portálu Azure a kopii hello základní disk virtuálního počítače a všechny připojené toohello virtuální pevné disky virtuálního počítače. To by zachovávají bod v bitové kopii čas hello virtuálního počítače a jeho přidružené disk. Toocopy hello "zálohování" se doporučuje na jiný účet úložiště Azure. Proto hello postup popsaný v kapitole [kopírování disků mezi účty úložiště Azure] [ planning-guide-5.4.2] by použití tohoto dokumentu.
Kromě hello vypnutí pomocí hello portálu Azure, jeden můžete také provést pomocí prostředí Powershell nebo rozhraní příkazového řádku podle postupu popsaného tady: <https://azure.microsoft.com/documentation/articles/virtual-machines-deploy-rmtemplates-powershell/>

Obnovení tohoto stavu by obsahovat odstranění hello základní virtuální počítač, jakož i hello původní disky hello základní virtuální počítač a připojit virtuální pevné disky, kopírování back hello uložit virtuální pevné disky toohello původní účet úložiště a pak opětovného nasazení hello systému.
...
Zkontrolujte, zda tooinstall novou licenci SAP vzhledem k tomu, že restoing zálohování virtuálních počítačů, jak je popsáno výše vytvoří nový klíč hardwaru.

### <a name="online-backup-of-an-sap-system"></a>Online zálohování systému SAP
Zálohování hello databázového systému se provádí pomocí konkrétní metody databázového systému, jak je popsáno v hello [databázového systému průvodce][dbms-guide].

Ostatní virtuální počítače v rámci hello systému SAP lze zálohovat pomocí funkce zálohování virtuálních počítačů Azure. Zálohování virtuálního počítače Azure získali zavedená již v rané fázi v 2015 a mezitím je standardní metoda toobackup dokončení virtuálního počítače v Azure. Zálohování Azure ukládá hello záloh v Azure a umožňuje obnovení virtuálního počítače znovu.
...
>
> tooget obeznámeni s zálohování virtuálních počítačů Azure, spusťte zde: <https://azure.microsoft.com/documentation/articles/backup-azure-vms/>.
>
> Další možnosti jsou toouse kombinaci aplikace Microsoft Data Protection Manager nainstalovat do virtuálního počítače Azure a Azure Backup k zálohování a obnovení databáze. Další informace naleznete zde: <https://azure.microsoft.com/documentation/articles/backup-azure-dpm-introduction/>.  
>
> ![Linux][Logo_Linux] Linux
>
> Není žádná ekvivalentní tooWindows služby Stínová kopie svazku v systému Linux. Pouze konzistentními soubory zálohy jsou proto možná, ale není konzistentní s aplikací zálohování. Hello databázového systému SAP zálohování by mělo být provedeno pomocí funkce databázového systému. Hello systému souborů, která zahrnuje hello SAP související data můžete uložit například pomocí vkládání popsaný tady: <http://help.sap.com/saphelp_nw70ehp2/helpdata/en/d3/c0da3ccbb04d35b186041ba6ac301f/content.htm>
>
>
