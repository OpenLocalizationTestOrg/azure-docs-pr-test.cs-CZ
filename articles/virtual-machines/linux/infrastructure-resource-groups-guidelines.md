...
* Vytvořte sadu hello skupin prostředků pomocí zásady vytváření názvů. Můžete použít hello rozhraní příkazového řádku Azure nebo portál.

## <a name="resource-groups"></a>Skupiny prostředků
V Azure můžete logicky seskupují související prostředky, například účty úložiště, virtuální sítě a virtuální počítače (VM) toodeploy, spravovat a udržovat je jako jedna entita. Tento přístup umožňuje snazší toodeploy aplikace při zachování hello všechny související prostředky společně z hlediska správy nebo toogrant ostatní přístup toothat skupiny prostředků. Názvy skupin prostředků, může být maximálně 90 znaků. Pro získat komplexnější přehled o skupinách prostředků, můžete si přečíst hello [přehled Azure Resource Manageru](../../azure-resource-manager/resource-group-overview.md).

Klíčovou funkcí tooResource skupiny je hello možnost toobuild prostředí pomocí souboru JSON, který deklaruje hello úložiště, sítě, a výpočetní prostředky. Můžete také definovat všechny související vlastní skripty nebo tooapply konfigurace. Pomocí těchto šablon JSON vytvářet konzistentní a reprodukovatelné nasazení pro vaše aplikace. Tento přístup umožňuje vytvořit prostředí při vývoji a potom pomocí této stejné šablony toocreate produkční nasazení, nebo naopak. Lépe porozumět tomu, o pomocí šablon, přečtěte si [hello názorný Průvodce šablonou](../../azure-resource-manager/resource-manager-template-walkthrough.md) který vás provede každého kroku vytváření šablony JSON.
