...
- V rámci SAP HANA s časovým razítkem infrastrukturu Azure (velké Instance) jsou mnoha různými klienty nasazení a izolované proti sobě prostřednictvím klienta koncepty na úrovni sítě, úložiště a výpočty. 
...
Tento dokument je jeden z pěti dokumentů, které zahrnují tématu SAP HANA v Azure (velké Instance). V tomto dokumentu jsme přejděte prostřednictvím základní architekturu a odpovědnosti, služeb a hlavní prostřednictvím funkce řešení. Pro většinu v oblastech, jako je vytváření sítí a připojení čtyři dokumenty nepřekrývají podrobnosti a přejít k podrobnostem seznamy. V dokumentaci SAP HANA v Azure (velké Instance) nezahrnuje aspekty SAP NetWeaver instalaci nebo nasazení SAP NetWeaver ve virtuálních počítačích Azure. Toto téma je zahrnuté v samostatné dokumentaci najít ve stejném kontejneru dokumentaci. 


Pět části této příručky zahrnovat následující témata:
...
Stejně jako, můžete si vybrat mezi různé typy virtuálních počítačů s virtuálními počítači Azure, můžete z různých SKU z HANA velké instancí, které jsou přizpůsobené pro různé zatížení typy SAP HANA. SAP platí paměti pro poměr soketu procesoru u různých úloh podle generace procesor Intel – čtyři různé typy SKU nabízí:
...
| --- | Na Azure S384 SAP HANA<br /> – 8 procesor Intel Xeon® x E7 8890 v4<br /> 192 jader procesoru a 384 procesoru vláken |  4.0 TB |  16 TB | Připraveno k seřazení |
...
|---| Na Azure S384m SAP HANA<br /> – 8 procesor Intel Xeon® x E7 8890 v4<br /> 192 jader procesoru a 384 procesoru vláken |  6.0 TB |  18 TB | Připraveno k seřazení |
|---| Na Azure S384xm SAP HANA<br /> – 8 procesor Intel Xeon® x E7 8890 v4<br /> 192 jader procesoru a 384 procesoru vláken |  8.0 TB |  22 TB |  Připraveno k seřazení |
|---| Na Azure S576 SAP HANA<br /> – Procesor Intel Xeon® x 12 E7 8890 v4<br /> 288 jader procesoru a 576 procesoru vláken |  12.0 TB |  28 TB | Připraveno k seřazení |
|---| Na Azure S768 SAP HANA<br /> – Procesor Intel Xeon® x 16 E7 8890 v4<br /> 384 jader procesoru a 768 procesoru vláken |  16.0 TB |  36 TB | Připraveno k seřazení |
|---| Na Azure S960 SAP HANA<br /> – 20 procesor Intel Xeon® x E7 8890 v4<br /> 480 jader procesoru a 960 procesoru vláken |  20.0 TB |  46 TB | Připraveno k seřazení |
...
Různé výše konfigurace, které jsou k dispozici nebo jsou 'nenabízí už' se odkazuje v [SAP podporu Poznámka #2316233 – SAP HANA v Microsoft Azure (velké instance)](https://launchpad.support.sap.com/#/notes/2316233/E). Konfigurace, které jsou označeny jako 'připravení pořadí, bude brzy najít jejich vstupu v Poznámka SAP. Ale tyto instance SKU lze provést řazení již pro šesti různých oblastech Azure HANA velké Instance služby je k dispozici.

Konkrétní konfigurace vybrali jsou závislé na zatížení, prostředky procesoru a požadovanou paměť. Je možné, pro zatížení OLTP využít SKU, které jsou optimalizované pro úlohy OLAP. 
...
**Práce v síti,** všechny interní sítě pro razítko velké Instance s SAP HANA, přístup k úložišti, připojení mezi instance (pro Škálováním na více systémů a další funkce), připojení k povahu a připojení k Azure, kde je aplikační vrstvu SAP hostované v Azure Virtual Machines. Zahrnuje také připojení k síti WAN mezi datovými centry Azure pro zotavení po havárii pro účely replikace. Všechny sítě jsou rozdělena na oddíly prostřednictvím klienta a provedli QOS.
...
Jako zákazník můžete použít snímky úložiště pro zálohování a obnovení a po havárii pro účely obnovení. Další informace o tomto tématu jsou podrobně popsané na [SAP HANA (velké instance) vysokou dostupnost a zotavení po havárii v Azure](hana-overview-high-availability-disaster-recovery.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json).
...
S typem I třídy SKU, logická jednotka je uložený ve, spouštěcí svazek je zašifrován. V případě typu třídy II SKU z HANA velké instancí je potřeba šifrovat spouštění logické jednotky s metody operačního systému. 
...
1. SAP HANA v Azure (velké instance) můžete přistupovat pouze virtuální počítače Azure pro vyhrazené připojení ExpressRoute; Ne přímo z místní. Někteří klienti správy a všechny aplikace, které vyžadují přímý přístup, jako je například SAP řešení správce spuštěného na místě, se nemůže připojit k databázi SAP HANA.
...
> [!NOTE] 
> Pokud potřebujete v připojení k SAP HANA v Azure (velké instance) _datového skladu_ scénář, kde aplikace nebo koncoví uživatelé musí připojit k databázi SAP HANA (spuštěna přímo), se musí použít jinou součást síťového: zpětného proxy pro data trasy, která do a z. Například F5 BIG-IP, NGINX s Traffic Manager nasazené v Azure jako virtuální brány firewall nebo provoz směrování řešení.
