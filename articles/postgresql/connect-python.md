...
- Uložte soubor s příponou .py (třeba postgres.py) do složky vašeho projektu. Pokud používáte operační systém Windows, nezapomeňte při ukládání souboru vybrat kódování UTF-8. 
- Spusťte Příkazový řádek nebo prostředí Bash a změňte adresář na složku vašeho projektu, například `cd postgres`.
