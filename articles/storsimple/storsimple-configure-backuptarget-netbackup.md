...
3.  Na portálu Azure classic, získat název prostředku a [registračního klíče služby StorSimple Manager](storsimple-deployment-walkthrough-u2.md#step-2-get-the-service-registration-key).
4.  Na serveru, který spouští skript spusťte prostředí PowerShell jako správce. Zadejte tento příkaz:

    `Get-AzureStorSimpleDeviceBackupPolicy –DeviceName <device name>`

    Poznamenejte si ID zásady zálohování.
5.  V poznámkovém bloku vytvořte nový skript prostředí PowerShell pomocí následující kód.
    ...
    ```
      Uložení skriptu prostředí PowerShell do stejného umístění, kam jste uložili vašeho Azure nastavení publikování. Například uložte jako C:\CloudSnapshot\StorSimpleCloudSnapshot.ps1.
6.  Přidáte skript do úlohy zálohování v NetBackup. K tomu upravte možnosti se úloha NetBackup předběžné zpracování a po zpracování příkazů.
