...
| TCP 9354 |na více systémů |WAN |Ano |Hello odchozí port je používán toocommunicate zařízení StorSimple hello s hello služby StorSimple Manager. |
...
| --- | --- | --- |
| `https://*.storsimple.windowsazure.com/*`<br>`https://*.accesscontrol.windows.net/*`<br>`https://*.servicebus.windows.net/*` |Služba StorSimple Manager<br>Access Control Service<br>Azure Service Bus |Povolenou podporu cloudu síťová rozhraní |
...
| Vzor adresy URL | Komponenta nebo funkce | IP adresy zařízení |
| --- | --- | --- |
| `https://*.storsimple.windowsazure.us/*`<br>`https://*.accesscontrol.usgovcloudapi.net/*`<br>`https://*.servicebus.usgovcloudapi.net/*` |Služba StorSimple Manager<br>Access Control Service<br>Azure Service Bus |Povolenou podporu cloudu síťová rozhraní |
...
Hello směrování metriky algoritmy se liší v závislosti na verzi softwaru hello spuštěné v zařízení StorSimple.

**TooUpdate předchozí verze 1**

To zahrnuje vydání předchozí tooUpdate 1 například hello GA 0,1, 0,2 nebo 0,3 verze softwaru. pořadí Hello založené na směrování metriky vypadá takto:
...
**Verze od Update 1 a předchozí tooUpdate 2**

To zahrnuje verze softwaru, jako je například 1, 1.1 nebo 1.2. pořadí Hello založené na směrování metriky je toto rozhodnutí:
   ...
   *DATA 0 > poslední nakonfigurované 10 GbE síťové rozhraní > ostatní 10 GbE síťové rozhraní > poslední nakonfigurované 1 GbE síťové rozhraní > ostatní 1 GbE síťové rozhraní*
   ...
   V Update 1 hello směrování metrika dat 0 je provedena hello nejnižší; proto všechny hello cloudu – provoz se směruje pomocí DATA 0. Poznamenejte si toho pokud se v zařízení StorSimple nachází více než jedno povolenou podporu cloudu síťové rozhraní.

**Verze od Update 2**

Aktualizace 2 má několik vylepšení souvisejícím se sítí a směrování metriky hello se změnila. Takto lze vysvětlit Hello chování.
...
* Hello pořadí, ve kterém budou směrovány přenosy dat cloudové hello přes hello síťových rozhraní je:
    ...
    *kde hello čísla v závorkách označují hello příslušných směrování metriky.*
...
* Výstrahu se také vyvolá zařízení StorSimple, když dojde k selhání VIP. Další informace, přejděte příliš[výstrahy Stručná referenční příručka](storsimple-manage-alerts.md).
...
* Izolujte přenosy iSCSI a cloudu hello nutnosti mít vyhrazené síťových rozhraní na vašem zařízení pro přístup k iSCSI a cloudu. Další informace najdete v tématu Jak příliš[upravit síťových rozhraní](storsimple-modify-device-config.md#modify-network-interfaces) zařízení StorSimple.
...
Další informace o monitorování a údržbu hello hardwarové součásti zařízení StorSimple, přejděte příliš[použít hello StorSimple Manager service toomonitor hardwarové součásti a stav](storsimple-monitor-hardware-status.md) a [StorSimple výměna hardwaru součást](storsimple-hardware-component-replacement.md).
...
* Při nahrazení, monitorovat hello **stavu hardwaru** v hello **údržby** stránky pro hello jednotek v hello SSD a HDD. Zelená zkontrolujte stav označuje, že hello disky jsou v pořádku nebo OK zatímco červený vykřičník bodu indikuje selhání SSD nebo pevný disk.
...
* Během EBOD řadiče modulu nahrazení, neustále monitorovat stav hello hello součásti v hello služby StorSimple Manager díky přístupu k **údržby** > **stavu hardwaru**.
...
* [Zjistěte, jak toodeploy vašeho řešení StorSimple](storsimple-deployment-walkthrough-u2.md).
