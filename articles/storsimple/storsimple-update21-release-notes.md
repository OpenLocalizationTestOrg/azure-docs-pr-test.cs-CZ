...
Hello následující poznámky k verzi popisují hello nové funkce a identifikovat hello kritické otevřené problémy pro StorSimple 8000 řady aktualizace 2.2. Také obsahují seznam aktualizací softwaru zařízení StorSimple hello součástí této verze. 

Aktualizace 2.2 může být zařízení StorSimple použité tooany systémem verze (GA) nebo Update 0.1 prostřednictvím Update 2.1. verze zařízení Hello přidružené 2.2 aktualizace je 6.3.9600.17708.

Přečtěte si hello informace obsažené v uvolněte hello poznámky před nasazením hello aktualizovat v řešení StorSimple.

> [!IMPORTANT]
> * Aktualizace 2.2 má pouze aktualizace softwaru. Tato aktualizace trvá přibližně tooinstall 1.5 – 2 hodiny. 
...
Hello následující klíčových vylepšení byly provedeny v 2.2 aktualizace.
...
* **Pořízení snímku vylepšení výkonu** – 2.2 aktualizace má lepší hello čas tooprocess cloudový snímek v některých scénářích, kde se používá velké svazky a je minimální toono data změn. Scénář, který bude využívat toto vylepšení by hello archivu svazky.
* **Posílení zabezpečení balíčku podporu shromažďování** – byly vylepšení způsobem hello balíček pro podporu hello je shromáždit a odeslat v této verzi. 
...
Následující tabulky Hello poskytuje shrnutí problémů, které byly odstraněny v 2.2 aktualizace a 2.1.    

| Ne | Funkce | Problém | Platí toophysical zařízení | Platí toovirtual zařízení |
| --- | --- | --- | --- | --- |
| 1 |Výkon hostitele |V hello starší verze, problémy s výkonem na straně hostitele se zjištěnými během vytváření hello místně vázaný svazek a během převodu hello tooa vrstvený svazek, který je místně připnutý svazku. V této verzi, což by vedlo k zlepšení výkonu hello hostitele během vytváření a převod svazku postupů hello opravě těchto problémů. |Ano |Ne |
| 2 |Místně vázaných svazků |Ve výjimečných případech by systému hello k chybě při vytváření místně vázaný svazek. Tato chyba byla opravena v této verzi. |Ano |Ne |
| 3 |Vrstvení |Když hello metadata pro vrstvené hello cloudu zařízení StorSimple (8010 a 8020) příliš hello cloudu se vyskytly Ojediněle dojde k chybě. Tento problém vyřešen v této verzi. |Ne |Ano |
| 4 |Vytvoření snímku |Byly problémy související toohello vytváření přírůstkové snímků ve scénářích s velké svazky a minimální toono data změn. V této verzi opravě těchto problémů. |Ano |Ano |
| 5 |Openstack ověřování |Při použití Openstack jako poskytovatele cloudových služeb hello, uživatel hello by spustit do nepravidelným chyby související toohello ověřování, kde hello JSON analyzátor výsledkem havárie. Tato chyba je opravena v této verzi. |Ano |Ne |
| 6 |Kopírování na straně hostitele |V dřívějších verzích softwaru nepravidelným chyby související s toohello ODX časování zobrazila při kopírování dat hello ze svazku tooanother jeden svazek. To by způsobilo selhání řadiče a hello systému může potenciálně přejděte do režimu obnovení. Tato chyba je opravena v této verzi. |Ano |Ne |
| 7 |Windows Management Instrumentation (WMI) |V předchozích verzích hello softwaru měla několik instancí webového proxy serveru chyba s výjimkou hello "<ManagementException> selhání načtení zprostředkovatele". Tato chyba byla navrácená paměť WMI s atributy tooa a nyní vyřešen. |Ano |Ne |
| 8 |Aktualizace |V určitých výjimečných případech v předchozích verzích hello softwaru hello uživatele přijat "CisPowershellHcsscripterror" při pokusu o tooscan nebo instalace aktualizací. Tento problém vyřešen v této verzi. |Ano |Ano |
| 9 |Balíček pro podporu |V této verzi byly vylepšení toohello způsob je shromáždit a odeslat balíček pro podporu hello. |Ano |Ano |
...
Tato verze obsahuje softwarové aktualizace. Ale při aktualizaci z předchozí tooUpdate verze 2, budete potřebovat tooinstall ovladač Storport, Spaceport a (v některých případech) na disku při aktualizacích firmwaru v zařízení.

Další informace o tom, jak tooinstall hello ovladačů, Storport, Spaceport a aktualizace firmwaru disku, najdete v [nainstalovat aktualizace 2.2](storsimple-install-update-21.md) zařízení StorSimple.
...
Zjistěte, jak příliš[nainstalovat aktualizace 2.2](storsimple-install-update-21.md) zařízení StorSimple.
